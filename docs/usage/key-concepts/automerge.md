...
This means merging multiple branches in a row does not work reliably, so we prefer not to do that.
...
Non-major updates in SemVer ecosystems should not have breaking changes (if they follow the spec), so many users enable automerge for these too:
...
If you do not want Renovate to use the platform-native automerge, then set `platformAutomerge` to `false`.
...
Confirm you have set the correct "required checks" for your base branch.
...
If you _do not_ use GitHub Actions as your CI provider, follow these steps:
...
If you have enabled branch protection which prevents Renovate from automerging directly to the base branch, then this does not work, and you should stick with the default PR-based automerging instead.
...
Note: Renovate does not add assignees and reviewers to a PR with failing checks if the PR already has assignees or reviewers present.
...
If you have protected your base branch with a list of allowed committers, and Renovate is not on that list, then naturally automerge can not work.
...
It does not matter where you have put the global config, the important point in this example is that you are extending from a global config that's somewhere else.
For this example we will assume you put your config in a repository on GitHub, called `.github`.
