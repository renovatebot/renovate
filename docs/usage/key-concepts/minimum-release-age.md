...
uv's `exclude-newer` setting accepts a friendly duration (for example, `"3 days"`), an ISO 8601 duration (for example, `"P7D"`), or an RFC 3339 timestamp; Renovate resolves all three formats to absolute dates before comparing.
...
For per-package overrides, use uv's native [`[tool.uv].exclude-newer-package`](https://docs.astral.sh/uv/reference/settings/#exclude-newer-package) setting in `pyproject.toml`.
uv merges those per-package values with the global `--exclude-newer` flag, so per-package cooldowns take precedence for the listed packages while Renovate's global value still applies to the rest.
