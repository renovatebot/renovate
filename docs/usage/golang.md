...
1. If the user has enabled `gomodTidyAll` in the [`postUpdateOptions`](./configuration-options.md#postupdateoptions), then Renovate runs `go mod tidy` on all dependent modules in dependency order (dependencies before dependents).
