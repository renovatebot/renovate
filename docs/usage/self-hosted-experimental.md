...
## `RENOVATE_X_FORGEJO_AUTO_MERGEABLE_CHECK_ATTEMPTS`

If set to a positive integer, Renovate uses this as the maximum number of times to check if a Forgejo pull request is mergeable before issuing the automerge call.

Default value: `5`.

## `RENOVATE_X_FORGEJO_AUTO_MERGEABLE_CHECK_DELAY`

If set, Renovate uses this as the base delay between mergeable checks on Forgejo. Each attempt waits longer than the last (`delay * attempt * attempt` milliseconds).
