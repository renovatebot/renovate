...
Once Renovate's manager is done scanning files and extracting dependencies, it will assign a `datasource` to each so that Renovate then knows how to search for new versions.
