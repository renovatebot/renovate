...
For troubleshooting, we recommend you increase the logging to `DEBUG` level by adding `LOG_LEVEL=debug` to your environment variables before invoking Renovate.
...
2. Find all relevant `DEBUG` or `INFO` messages from before and after the problem occurred
3. Copy/paste the relevant parts of the logs into your discussion post or bug report
...
If later it turns out that the full logs are necessary, you'll be asked for them then.

## Validating configuration changes

Sometimes you'll have to change your Renovate configuration to solve a problem.
