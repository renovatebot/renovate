...
Uses the URL-embedded credentials by default or when set to `url`, or the [Git Credential Store](https://git-scm.com/docs/git-credential-store) when set to `store`. The Renovate Bot creates or updates the `~/.git-credentials` file when it does not contain the GitLab URL with the current username and password. Please note that the bot does not revert the changes when finishing.
