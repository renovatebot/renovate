...
If you wish to limit Renovate to apply configuration rules to certain files in the root repository directory, you have to use `matchFileNames` with a `minimatch` pattern (which can include an exact file name match) or an RE2 regex pattern.
...
Renovate will compare `matchFileNames` glob or RE2 regex matching against the dependency's package file and also lock file if one exists. A regex pattern must be wrapped in `/`.
