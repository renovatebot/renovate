...
| `composerGitlabToken`    | Disable setting `gitlab-token` ([docs](https://getcomposer.org/doc/articles/authentication-for-private-packages.md#gitlab-token)) and `gitlab-domains` ([docs](https://getcomposer.org/doc/06-config.md#gitlab-domains)) inside composer config |
