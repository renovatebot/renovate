...
| `gomodTidyAll`               | After running `go mod tidy` on the updated module, also run it on every other `go.mod` that references it via a local `replace` directive, in dependency order. Useful for Go monorepos where a shared library's `go.sum` update must propagate to dependents. Implies `gomodTidy`.                                                                                                                                                                                                |
