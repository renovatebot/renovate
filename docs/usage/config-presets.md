...
When repository onboarding happens, if a repository called `renovate-config` exists under the same organisation, that repository will be suggested as the preset instead of `config:base`. This allows for a seamless onboarding workflow with your own organisation settings.
