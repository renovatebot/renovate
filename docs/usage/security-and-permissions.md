...
- when `mise lock` is run from the repository checkout with trusted config paths enabled (if setting [`allowedUnsafeExecutions=["mise"]`](./self-hosted-configuration.md#allowedunsafeexecutions).
