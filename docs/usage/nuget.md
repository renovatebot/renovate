...
1. Existing dependencies will be extracted from `<PackageReference>` and `<PackageVersion>` tags
1. Renovate will look up the latest version on [nuget.org](https://nuget.org) (or on [alternate feeds](#Alternate%20feeds)) to determine if any upgrades are available
