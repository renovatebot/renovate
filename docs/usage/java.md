...
- Catalogs outside the `gradle` folder, whose names do _not_ end in `.versions.toml` (unless overridden via [`filePatterns`](./configuration-options.md#filepatterns) configuration)
