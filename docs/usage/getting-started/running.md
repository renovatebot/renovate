...
Renovate's server-side/admin config is referred to as its "global" config, and can be specified using either a config file, environment variables, or CLI parameters. Renovate autodetects a global config file named `config.js`. Other file types (`*.js`, `*.json`, `*.json5`, `*.yaml`, `*.yml`) are supported, when you reference them with the `RENOVATE_CONFIG_FILE` environment variable (e.g. `RENOVATE_CONFIG_FILE=config.yaml`).
