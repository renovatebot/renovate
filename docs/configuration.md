...
| `prCreation` | When to create the PR for a branch | string | `"immediate"` | `RENOVATE_PR_CREATION` | `--pr-creation` |
| `automerge` | What types of upgrades to merge to base branch automatically. Values: none, patch, minor or any | string | `"none"` | `RENOVATE_AUTOMERGE` | `--automerge` |
| `prMergeMethod` | Method of merging to perform. GitHub: merge, squash, rebase. | string | `"merge"` | `RENOVATE_PR_MERGE_METHOD` | `--pr-merge-method` |
