...
Also, make sure to set the `GITHUB_TOKEN` in your environment as we use `gh` to fetch the issues. If you wish to skip fetching the issues, then set `SKIP_GITHUB_ISSUES` to `true` before building the docs.
