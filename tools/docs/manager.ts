...
export async function generateManagers(
  ...
  for (const [manager, definition] of allManagers) {
    ...
    const { filePatterns } = defaultConfig as RenovateConfig;
    ...
    if (!isCustomMgr) {
      ...
      md += `For details on how to extend a manager's \`filePatterns\` value, please follow [this link](../index.md#file-matching).\n\n`;
