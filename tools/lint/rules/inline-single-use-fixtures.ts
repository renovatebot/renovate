 ...
 */
function isRelocatableInitializer(init: ESTree.Expression): boolean {
  if (
 ...
 */
function getCalleeBaseName(callee: ESTree.Node): string | null {
  let current = callee;
  for (;;) {
    switch (current.type) {
      case 'Identifier':
  ...
  createOnce(context) {
    return {
      ...
      VariableDeclaration(node) {
        ...
        if (initLoc.end.line <= initLoc.start.line) {
          return;
        }
        ...
        let testCall: ESTree.CallExpression | null = null;
        for (const reference of references) {
          const enclosing = findEnclosingTestCall(reference.identifier);
