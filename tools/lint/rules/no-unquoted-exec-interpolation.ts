...
/**
 * Child links probed by `mentionsQuote`'s generic walk. Each key exists on only
 * some node types, so the discriminated `ESTree.Node` union cannot be indexed
 * by them; this reduced view describes just what that walk reads.
 */
interface NodeChildren {
  type: string;
  arguments?: ESTree.Node[];
  body?: ESTree.Node | ESTree.Node[] | null;
  callee?: ESTree.Node | null;
  /**
   * On `ArrowFunctionExpression` this is a boolean flag rather than a child
   * node; the walk's `typeof !== 'object'` guard filters it out.
   */
  expression?: ESTree.Node | boolean | null;
  object?: ESTree.Node | null;
}
 ...
 */
function readName(input: ESTree.Node | undefined): string | null {
 ...
 */
function mentionsQuote(
  node: ESTree.Node | boolean | null | undefined,
): boolean {
 ...
 */
function isQuoted(input: ESTree.Node | undefined): boolean {
  const node = unwrap(input);
 ...
 */
function readUntrustedNames(input: ESTree.Node | undefined): string[] {
  const node = unwrap(input);
  ...
  const callee = unwrap(node.callee);
  const candidates: (ESTree.Node | undefined)[] = [...node.arguments];
  ...
  const found = candidates
    ...
    .filter(
      (name): name is string => name !== null && UNTRUSTED_NAMES.has(name),
    );
  ...
  createOnce(context) {
    return {
      TemplateLiteral(node) {
        if (!isCommandContext(node, node.parent)) {
          return;
        }
