...
async function newExtractDependencies(config) {
  ...
  for (const manager of managerList) {
    ...
    if (managers[manager].postExtractDependencies) {
      managerFiles = managers[manager].postExtractDependencies(managerFiles);
    }
    packageFiles = packageFiles.concat(managerFiles);
  }
