...
const managers = {};
const managerList = ['bazel', 'docker', 'meteor', 'node', 'npm'];
...
function getPackageUpdates(config) {
  ...
  if (manager === 'docker') {
    return managers.docker.getPackageUpdates(config);
  } else if (manager === 'npm') {
    ...
    return managers.node.getPackageUpdates(config);
  } else if (manager === 'bazel') {
    return managers.bazel.getPackageUpdates(config);
  }
