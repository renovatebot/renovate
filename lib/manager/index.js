...
async function detectPackageFiles(config) {
  ...
  for (const manager of managerList) {
    ...
    const files = [];
    const fileMatch = new RegExp(config[manager].fileMatch);
    logger.debug(`Using ${manager} file pattern: ${fileMatch.toString()}`);
    const allfiles = fileList.filter(file => file.match(fileMatch));
...
function getManager(config, filename) {
  for (const manager of managerList) {
    if (filename.match(new RegExp(config[manager].fileMatch))) {
      return manager;
    }
  }
  return null;
