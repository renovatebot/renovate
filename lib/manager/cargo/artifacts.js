...
async function getArtifacts(
  ...
  try {
    ...
    for (let i = 0; i < updatedDeps.length; i += 1) {
      ...
      const cmd = `cargo update --manifest-path ${localPackageFileName} --package ${dep}`;
      // if (config.binarySource === 'docker') {
      //   cmd = `docker run renovate/cargo update --manifest-path ${localPackageFileName} --package ${dep}`;
      // }
...
async function updateAll(localPackageFileName) {
  ...
  // if (config.binarySource === 'docker') {
  //   cmd = 'docker run renovate/cargo update --manifest-path ' + localPackageFileName;
  // }
  const startTime = process.hrtime();
