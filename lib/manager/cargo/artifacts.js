...
async function getArtifacts(
  ...
  lockFileName = packageFileName.replace(/\.toml$/, '.lock');
  try {
    await fs.outputFile(packageFileName, newPackageFileContent);
    ...
    for (let i = 0; i < updatedDeps.length; i += 1) {
      ...
      const cmd =
        'cargo update --manifest-path' + packageFileName + ' -p ' + spec;
    ...
    const newCargoLockContent = await fs.readFile(lockFileName, 'utf8');
