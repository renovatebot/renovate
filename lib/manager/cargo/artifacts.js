...
function createMainrs(localDir) {
  fs.readdirSync(localDir).forEach(file => {
    const fileName = upath.join(localDir, file);
    if (fs.lstatSync(fileName).isDirectory()) {
      createMainrs(fileName);
    } else if (file == 'Cargo.toml') {
      const mainrs = upath.join(localDir, 'src/main.rs');
      logger.trace('Created file ', fileName);
      fs.ensureFile(mainrs);
    }
  });
}
