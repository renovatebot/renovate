...
function determineUpgrades(npmDep, config) {
  ...
  let rangeOperator;
  if (config.upgradeInRange && isValidRange(currentVersion)) {
  ...
  if (isRange(currentVersion)) {
    let newVersion;
    if (pinVersions && lockedVersion && isValidVersion(lockedVersion)) {
...
function isRange(input) {
  // Pinned versions also return true for isValidRange
  // We need to check first that they're not 'valid' to get only ranges
  return isValidVersion(input) === null && isValidRange(input) !== null;
}
