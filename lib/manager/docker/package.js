...
async function getPackageUpdates(config) {
  const { depName, currentFrom, currentTag, currentDigest, logger } = config;
  const upgrades = [];
  ...
  if (currentTag) {
    ...
    for (const newVersionMajor of Object.keys(versionUpgrades)) {
      ...
      upgrade.newFrom = upgrade.newDepTag;
      ...
      if (config.currentDigest || config.pinDigests) {
        ...
        upgrade.newFrom = `${upgrade.newFrom}@${upgrade.newDigest}`;
      }
      upgrades.push(upgrade);
      logger.info(
        { currentFrom: config.currentFrom, newFrom: upgrade.newFrom },
