const { getMajor, isValidVersionOrRange } = require('../../util/semver');
...
async function getPackageUpdates(config) {
  ...
  if (currentTag) {
    ...
    if (!isValidVersionOrRange(tagVersion)) {
      logger.info(
        { currentDepTag },
    ...
    const currentMajor = getMajor(padRange(tagVersion));
      ...
      versionList = allTags
        ...
        .filter(
          version =>
            ...
            (!currentlyStable && getMajor(padRange(version)) === currentMajor)
        )
        .filter(
    ...
    for (const version of versionList) {
      ...
      const newVersionMajor = getMajor(paddedVersion);
      let type;
    ...
    for (const upgradeKey of Object.keys(versionUpgrades)) {
      ...
      const newVersionMajor = `${getMajor(
