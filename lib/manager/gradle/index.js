...
async function extractAllPackageFiles(config, packageFiles) {
  ...
  gradle.init();
  gradleExecutorDeps = undefined;

  const gradleFiles = [];
  for (const packageFile of packageFiles) {
    const content = await platform.getFile(packageFile);
    if (content) {
      const dependencies = await dependencyExtractor(config, content);
...
async function dependencyExtractor(config, content) {
  const dependencies = [];
  if (config.gradle.useJsParser) {
    dependencies.push(...gradleParser.parseBuildGradle(content));
  }
  if (!config.gradle.disableGradleExecution) {
    dependencies.push(...(await dependencyExtractUsingGradle(config)));
  }
  return dependencies;
}

async function dependencyExtractUsingGradle(config) {
  if (gradleExecutorDeps) {
    return gradleExecutorDeps;
  }
  await updatesReport.createRenovateGradlePlugin(config.localDir);
  await executeGradle(config);

  gradleExecutorDeps = await updatesReport.extractDependenciesFromUpdatesReport(
    config.localDir
  );
  if (gradleExecutorDeps.length === 0) {
    return [];
  }
  return gradleExecutorDeps;
}
