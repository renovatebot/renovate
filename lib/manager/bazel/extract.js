...
function findBalancedParenIndex(longString) {
  ...
  return [...longString].findIndex(char => {
    record.push(char);
    ...
    switch (char) {
      ...
      case ')':
        ...
        break;
      case '"':
        if (record.slice(-2).every(recorded => char === recorded))
          intShouldNotBeOdd++;
    }
...
function parseContent(content) {
  ...
  const httpDefs = suffixBases
    .map(base => {
      const closingParenIndex = findBalancedParenIndex(base);

      return (
        Number.isInteger(closingParenIndex) &&
        `${prefix}${base.slice(0, closingParenIndex)})`
      );
    })
    .filter(Boolean);
