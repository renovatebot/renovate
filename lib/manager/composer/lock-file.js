...
async function getLockFile(
  ...
  let stderr;
  try {
    const localPackageFileName = upath.join(tmpDir.path, 'composer.json');
    ...
    const localLockFileName = upath.join(tmpDir.path, 'composer.lock');
    ...
    if (credentials && credentials.token) {
      ...
      const localAuthFileName = upath.join(tmpDir.path, 'auth.json');
      fs.outputFile(localAuthFileName, JSON.stringify(authJson));
    }
