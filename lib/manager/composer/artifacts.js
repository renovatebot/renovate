  ...
  try {
    ...
    if (hostOpts && hostOpts.token) {
      authJson['github-oauth'] = {
        'github.com': hostOpts.token,
      };
    }
    hostOpts = hostRules.find({
      hostType: 'gitlab',
        ...
        for (const regUrl of config.registryUrls) {
          if (regUrl.url) {
            ...
            if (hostOpts.username && hostOpts.password) {
              const host = URL.parse(regUrl.url).hostname;
              logger.debug('Setting packagist auth for host ' + host);
              authJson = authJson || {};
              authJson['http-basic'] = authJson['http-basic'] || {};
              authJson['http-basic'][host] = {
                username: hostOpts.username,
                password: hostOpts.password,
              };
            } else {
