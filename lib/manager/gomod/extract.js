...
function getDep(lineNumber, match) {
  ...
  if (!isVersion(currentValue)) {
    ...
    // Got unexpected rollbacks in testing so disabling rollbacks for gopkg.in
    dep.rollbackPrs = false;
    ...
    // TODO: dep.purl = `pkg:go/${depName}`
    dep.skipReason = 'unsupported-host';
  }
  if (dep.purl && currentValue.match(/^v0.0.0-\d+-[0-9a-f]+$/)) {
    logger.debug(`Dep ${depName} is unversioned - disabling rollback attempts`);
    dep.rollbackPrs = false;
  }
  return dep;
}
