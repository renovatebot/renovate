/**
 * TODO remove theese comments after conversation resolved.
 * I think it is good Idea to move up file hierarchy all primitives
 * like strings literals, booleans and numbers.
 * Store enums in one place (like Android framework demands)
 * will enable you to make changes easily in the future.
 * It will be nice to declare [multiStageDockerBuildEnums] (and in the
 * long term all other strings literals) inside the
 * enums responsible file of the project and then import
 * here as [m], then access value in concise way e.g. [m.STAGE].
 * Just by inspecting enums file developer will be able to quick
 * understand all possible states of entity without scouring the project.
 *
 * <naming is hard>
 */
const multiStageDockerBuildEnums = {
  STAGE: 'stage',
  FINAL: 'final',
};
...
function getDep(currentFrom) {
  ...
  dep.depType = multiStageDockerBuildEnums.STAGE;
  if (
    dep.depName &&
...
function extractPackageFile(content) {
  ...
  deps[deps.length - 1].depType = multiStageDockerBuildEnums.FINAL;
  return { deps };
}
