...
function updateDependency(upgrade) {
  ...
  function ensureBranch() {
    ...
    return github.getFile(config.packageFile, branchName)
    .then((res) => {
      logger.debug(`Found existing branch ${branchName}`);
      return res;
    })
    .catch(() => {
      parentBranch = undefined;
      // Get the base branch version instead
      logger.debug(`No existing branch ${branchName} found`);
      return github.getFile(config.packageFile);
    })
    ...
    .then((fileContents) => {
      ...
      logger.debug(`${depName}: Adding commit '${commitMessage}'`);
      // If this is our first commit then `parentBranch` will be undefined
      return github.commitFile(config.packageFile, fileContents, commitMessage, parentBranch);
    })
    .then((commit) => {
      if (!commit) {
        return Promise.resolve();
      }
      return github.setBranchCommit(branchName, commit);
    });
  }
