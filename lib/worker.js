...
function updateDependency(upgrade) {
  ...
  function ensureBranchAndCommit() {
    ...
    .then((res) => {
      if (res) {
        ...
        branchExists = false;
        parentBranch = github.getDefaultBranch();
        logger.debug(`Branch ${branchName} needs creation`);
      }
      return Promise.resolve();
    })
    // Get existing package.json
    .then(() => github.getFile(config.packageFile, parentBranch))
    // Check if package.json needs updating
    .then((res) => {
      logger.silly(`Existing ${config.PackageFile}:`);
      logger.silly(res.body.content);
      ...
      if (currentJson[depType][depName] === newVersion) {
        // This should never happen unless the branch exists
        logger.verbose(`${depName}: branch ${parentBranch} is already up-to-date`);
        return Promise.resolve();
      }
      logger.verbose(`${depName}: Updating to ${newVersion}`);
    ...
    .then((fileContents) => {
      // No need to write commit if no contents
      if (!fileContents) {
        return Promise.resolve();
      }
      logger.silly(`New ${config.packageFile} contents:`);
      logger.silly(fileContents);
      logger.debug(`Adding commit '${commitMessage}' on top of branch ${parentBranch}`);
      return github.commitFile(parentBranch, config.packageFile, fileContents, commitMessage);
    })
    .then((commit) => {
      if (!commit) {
        return Promise.resolve();
      ...
      if (branchExists) {
        logger.debug(`Updating branch ${branchName} with new commit`);
        return github.updateBranch(branchName, commit);
      }
      logger.debug(`Creating branch ${branchName}`);
      return github.createBranch(branchName, commit);
    });
  }
