...
function checkIfConfigured() {
  logger.debug('Checking if repository is configured');
  // Search for closed PR "Configure renovate"
  return github.findPr('renovate/configure', 'Configure Renovate', 'closed')
  .then((closedPrFound) => {
    if (closedPrFound) {
    ...
    // If not found, check repoConfigured
    // If not found, look for any existing repo PRs
    // Close it if found
    // Else reject promise
    logger.debug('Repository not configured - rejecting');
