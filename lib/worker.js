...
async function updateDependency(upgrade) {
  ...
  const commitMessage = handlebars.compile(config.commitMessage)(params);
  ...
  try {
    ...
    await branchWorker.ensureBranch(config, branchName, depType, depName,
      newVersion, commitMessage);
    const upgradeConfig = Object.assign({}, config, upgrade);
    await prWorker.ensurePr(upgradeConfig);
  } catch (error) {
  ...
  async function checkForClosedPr() {
    ...
    const prExisted = await config.api.checkForClosedPr(branchName, prTitle);
