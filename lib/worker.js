  ...
  async function ensurePr(upgradeWithChangeLog) {
    ...
    try {
      ...
      if (existingPr) {
        ...
        const newPr = await createPr(branchName, prTitle, prBody);
        logger.info(`${depName}: Created ${newPr.displayNumber}`);
      }
    } catch (error) {
    ...
    async function checkExistingPr(pr) {
      ...
      if (pr.title === prTitle && pr.body === prBody) {
        logger.verbose(`${depName}: PR #${pr.number} already up-to-date`);
        return;
      }
      // PR must need updating
      logger.debug(`updatePr: ${pr.number}`);
      await api.updatePr(pr.number, prTitle, prBody);
      logger.info(`${depName}: Updated ${pr.displayNumber}`);
    }
  }
}

async function createPr(branchName, prTitle, prBody) {
  ...
  if (config.assignees.reviewers > 0) {
