...
async function updateDependency(upgrade) {
  ...
  async function getBranchPr() {
    const branchPr = await api.getBranchPr(branchName);
    if (branchPr) {
      logger.debug(`Found open PR for ${branchName}`);
      return api.getPr(branchPr.number);
    }
  ...
  async function updateExistingBranch() {
    ...
    const commit =
      await api.commitFile(config.packageFile, newContent, commitMessage, parentBranch);
    if (!commit) {
      return;
    }
    await api.updateBranch(branchName, commit);
  }
  ...
  async function createNewBranch() {
    ...
    const commit = await api.commitFile(config.packageFile, newContent, commitMessage);
    await api.createBranch(branchName, commit);
  }
  ...
  async function ensurePr(upgradeWithChangeLog) {
    ...
    async function createPr() {
      const newPr = await api.createPr(branchName, prTitle, prBody);
