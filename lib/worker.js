...
async function updateDependency(upgrade) {
  ...
  function getYarnLockFileName(packageFileName) {
    const packagePath = packageFileName.split('/');
    // Remove 'package.json'
    const basePath = packagePath.slice(0, -1);
    basePath.push('yarn.lock');
    return basePath.join('/');
  }

  function generateLockFile(newPackageJson) {
    logger.debug('Generating new yarn.lock file');
    rimraf.sync('/tmp/renovate-yarn');
    mkdirp.sync('/tmp/renovate-yarn');
    fs.writeFileSync('/tmp/renovate-yarn/package.json', newPackageJson);
    cp.spawnSync('yarn', ['install'], { cwd: '/tmp/renovate-yarn' });
    return fs.readFileSync('/tmp/renovate-yarn/yarn.lock');
  }
  ...
  async function createNewBranch() {
    ...
    const yarnLockFileName = getYarnLockFileName(config.packageFile);
    if (api.getFileContent(yarnLockFileName)) {
      logger.warn('Found yarn.lock file');
