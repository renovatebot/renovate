...
async function processYarnLock(inputConfig) {
  ...
  if (existingYarnLock === newYarnLock.contents) {
    logger.debug('Yarn lock does not need updating');
    return;
  }
  logger.debug('Yarn lock needs updating');
  const commitMessage = 'Regenerate yarn.lock file';
  // API will know whether to create new branch or not
  const params = Object.assign({}, inputConfig);
  params.branchName = 'renovate/yarn-lock';
  params.prTitle = 'Renovate yarn.lock';
  params.prBody = 'This PR regenerates yarn.lock files based on the existing `package.json` files.';
  await inputConfig.api.commitFilesToBranch(config.branchName, [newYarnLock], commitMessage);
