...
async function renovate(repoName, packageFile, packageConfig) {
  ...
  packageContentRaw = await api.getFileContent(packageFile);
  ...
  // Yarn
  const yarnLockPath = packageFile.replace('package.json', 'yarn.lock');
  logger.debug(`Looking for ${yarnLockPath}`);
  yarnLock = await api.getFileContent(yarnLockPath);

  if (yarnLock) {
    config.pinOnly = true;
    config.yarn = true;
    logger.verbose('Found yarn.lock file');
    const regex = /\n"?([^\s]+)@.*:\n\s+version "(.*?)"/g;
    const yarnPackages = {};
    let matches = regex.exec(yarnLock);
    while (matches) {
      yarnPackages[matches[1]] = matches[2];
      matches = regex.exec(yarnLock);
    }
    logger.silly(yarnPackages);
  } else {
    logger.debug('No yarn.lock found');
  }
...
async function processUpgradesSequentially(upgrades) {
  ...
  for (const upgrade of upgrades) {
    if (config.yarn) {
      await updateYarnLockFile(upgrade);
    } else {
...
async function updateYarnLockFile(upgrade) {
  const depName = upgrade.depName;
  const currentVersion = upgrade.currentVersion;
  rimraf.sync('/tmp/renovate-yarn');
  mkdirp.sync('/tmp/renovate-yarn');
  fs.writeFileSync('/tmp/renovate-yarn/package.json.old', packageContentRaw);
  fs.writeFileSync('/tmp/renovate-yarn/package.json', packageContentRaw);
  fs.writeFileSync('/tmp/renovate-yarn/yarn.lock.old', yarnLock);
  fs.writeFileSync('/tmp/renovate-yarn/yarn.lock', yarnLock);
  cp.spawnSync('yarn', ['upgrade', `${depName}@${currentVersion}`], { cwd: '/tmp/renovate-yarn' });
}
  ...
  async function ensurePr(upgradeWithChangeLog) {
    ...
    async function processExistingPr(existingPr) {
      if (!existingPr) {
        ...
        await addAssignees(pr.number);
        await addReviewers(pr.number);
        logger.info(`${depName}: Created ${pr.displayNumber}`);
        return;
      }
