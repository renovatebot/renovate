  ...
  describe('extractPackageFile()', () => {
    ...
    it('extracts multiple step pipeline image lines', () => {
      ...
      expect(res?.deps).toMatchObject([
        { currentValue: '1.13', depName: 'golang' },
        { currentValue: '10.0.0', depName: 'node' },
      ]);
    });
    ...
    it('extracts multiple services pipeline image lines', () => {
      ...
      expect(res?.deps).toMatchObject([
        { currentValue: '10.0.0', depName: 'node' },
        { currentValue: '5.7.24', depName: 'mysql' },
        { currentValue: 'alpine', depName: 'redis' },
      ]);
    });
    ...
    it('extracts multiple secrets pipeline image lines', () => {
      ...
      expect(res?.deps).toMatchObject([
        { currentValue: '10.0.0', depName: 'node' },
        { currentValue: 'v0.1.0', depName: 'target/secret-vault' },
      ]);
    });
  });
});
