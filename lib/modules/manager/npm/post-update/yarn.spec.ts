...
describe('modules/manager/npm/post-update/yarn', () => {
  ...
  it('patches local yarn (docker)', async () => {
    ...
    expect(execSnapshots).toMatchObject([
      { cmd: 'docker pull ghcr.io/containerbase/sidecar:8.0.2', options },
      {
        cmd:
          `docker run --rm --name=renovate_sidecar --label=renovate_child -v ".":"." -v "/tmp/cache":"/tmp/cache" -e CI -e BUILDPACK_CACHE_DIR -e CONTAINERBASE_CACHE_DIR -w "some-dir" ghcr.io/containerbase/sidecar:8.0.2 ` +
          `bash -l -c "` +
