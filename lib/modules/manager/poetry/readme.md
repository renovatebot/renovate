...
Enable [`splitPythonMarkers`](../../../configuration-options.md#splitpythonmarkers) to receive independent updates for variants of the same dependency that target different Python versions. This supports standalone PEP 508 `python_version` and `python_full_version` markers in project dependencies, as well as Poetry multiple-constraint arrays whose entries specify `version` and `python`.
