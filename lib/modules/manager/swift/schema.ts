...
const PackageResolvedPin = DeepNullish(
  z.object({
    ...
    location: z.string(),
    state: z.object({
      // `revision` is required for source-control pins (kind:
      // "remoteSourceControl"),
      // but absent for SwiftPM Package Registry pins (kind: "registry") per
      // SE-0292:
      // registry packages are identified by version alone, not by a commit SHA.
      // A
