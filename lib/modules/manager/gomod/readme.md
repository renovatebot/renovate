...
1. `goGenerate` - to run `go generate ./...` after vendoring. Will only run if the `allowedUnsafeExecutions` global option includes `goGenerate`. This will not install any additional tools required to run `go generate`, you can use [tools](https://tip.golang.org/doc/go1.24#tools) as one option. It is recommended to pair this with `gomodTidy` to ensure generate does not fail for an inconsistent sum file. Refer to the [configuration documentation for more information](../../../self-hosted-configuration.md#allowedunsafeexecutions).
