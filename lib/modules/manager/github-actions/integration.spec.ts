...
describe('modules/manager/github-actions/integration', () => {
  ...
  it('proposes updates for SHA-pinned action with major-only comment', async () => {
    ...
    getGithubDigest
      .mockResolvedValueOnce('aaaaaaaabbbbbbbbccccccccddddddddeeeeeeee11111111') // v6.1.0
  ...
  it('proposes updates for SHA-pinned action with major.minor comment', async () => {
    ...
    // getDigest is called once per update (major version + digest update)
    getGithubDigest
      .mockResolvedValueOnce('bbbbbbbbcccccccccceeeeeeeeefffffaaaaaa1111111112') // v5.1.0
      .mockResolvedValueOnce('aaaaaaaabbbbbbbbccccccccddddddddeeeeeeee11111111') // v6.1.0
      .mockResolvedValueOnce(
        'ffffffffffffffffffffffffffffffffffffffff22222222',
      ); // digest (v5)
  ...
  it('proposes updates for SHA-pinned action with full semver comment', async () => {
    ...
    const dep = extracted!.deps.find((d) => d.depName === 'astral-sh/setup-uv');
    expect(dep).toBeDefined();
    ...
    getGithubDigest
      .mockResolvedValueOnce('aaaaaaaabbbbbbbbccccccccddddddddeeeeeeee11111111') // v7.6.0
      .mockResolvedValueOnce('ffffffffffffffffffffffffffffffffffffffff22222222') // v8.0.0
      .mockResolvedValueOnce(
  ...
  it('proposes minor+major+digest updates for SHA-pinned with floating major comment', async () => {
    ...
    getGithubDigest
      .mockResolvedValueOnce('aaaaaaaabbbbbbbbccccccccddddddddeeeeeeee11111111') // v8.1.0 major
      .mockResolvedValueOnce(
        'ffffffffffffffffffffffffffffffffffffffff22222222',
      ); // digest (v7)
