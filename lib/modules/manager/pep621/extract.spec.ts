...
describe('modules/manager/pep621/extract', () => {
  describe('extractPackageFile()', () => {
    ...
    it('should not set constraints when splitPythonMarkers is disabled', async () => {
      ...
      const dep = result?.deps.find((d) => d.packageName === 'pytest');
      expect(dep?.extractedConstraints).toBeUndefined();
    ...
    it('should set constraints on fixture deps with markers when splitPythonMarkers is enabled', async () => {
      ...
      const tomli = result?.deps.find(
        (d) =>
          d.packageName === 'tomli' &&
          d.additionalBranchPrefix === 'py-lt-3-dot-11-',
      );
      expect(tomli).toMatchObject({
        packageName: 'tomli',
        currentValue: '>=1.1.0',
        extractedConstraints: { python: '>=3.7,<3.11' },
        constraintsFiltering: 'strict',
        additionalBranchPrefix: 'py-lt-3-dot-11-',
      ...
      expect(importlibMetadata).toMatchObject({
        packageName: 'importlib-metadata',
        currentValue: '>=3.6',
        extractedConstraints: { python: '>=3.7,<3.10' },
        constraintsFiltering: 'strict',
        additionalBranchPrefix: 'py-lt-3-dot-10-',
        commitMessageSuffix: '(python <3.10)',
      });
      ...
      const typingExt = result?.deps.find(
        (d) =>
          ...
          d.additionalBranchPrefix === 'py-lt-3-dot-8-',
      );
      expect(typingExt).toMatchObject({
        skipReason: 'unspecified-version',
        extractedConstraints: { python: '>=3.7,<3.8' },
        constraintsFiltering: 'strict',
        additionalBranchPrefix: 'py-lt-3-dot-8-',
      ...
      expect(packaging?.extractedConstraints).toBeUndefined();
    ...
    it('should handle python_full_version markers with splitPythonMarkers', async () => {
      ...
      expect(dep).toMatchObject({
        packageName: 'tomli',
        currentValue: '>=1.1.0',
        extractedConstraints: { python: '>=3.10.0' },
        constraintsFiltering: 'strict',
        additionalBranchPrefix: 'py-gt-eq-3-dot-10-dot-0-',
