...
describe('modules/manager/pep621/processors/uv', () => {
  ...
  describe('updateArtifacts()', () => {
    ...
    it.each([
      ...
      ['no --output-file flag', '--frozen --no-dev'],
    ])(
    ...
    it('uvExportRequirements: skips export when requirements.txt is empty', async () => {
      const execSnapshots = mockExecAll();
      GlobalConfig.set(adminConfig);
      mockLockAndRequirementsSetup();
    ...
    it('uvExportRequirements: skips export when --output-file does not match requirements.txt filename', async () => {
      const execSnapshots = mockExecAll();
      GlobalConfig.set(adminConfig);
      mockLockAndRequirementsSetup();
      fs.readLocalFile.mockResolvedValueOnce(
        '# This file was autogenerated by uv via the following command:\n' +
          '#    uv export --frozen --output-file other-requirements.txt\n' +
          'somepackage==1.0.0\n',
      );
      ...
      expect(logger.logger.debug).toHaveBeenCalledWith(
        {
          requirementsFileName: 'requirements.txt',
          outputFile: 'other-requirements.txt',
        },
        'uv export: --output-file does not match requirements.txt filename',
      );
    });

    it('uvExportRequirements: handles --output-file in = form', async () => {
      const execSnapshots = mockExecAll();
      GlobalConfig.set(adminConfig);
      mockLockAndRequirementsSetup();
      const requirementsWithHeader =
    ...
    it('uvExportRequirements: skips export when requirements.txt is not in same directory as pyproject.toml', async () => {
      const execSnapshots = mockExecAll();
      GlobalConfig.set(adminConfig);
      mockLockAndRequirementsSetup('subdir/requirements.txt');
      fs.getParentDir.mockReturnValueOnce('subdir').mockReturnValueOnce('');
      ...
      expect(logger.logger.debug).toHaveBeenCalledWith(
        {
          requirementsFileName: 'subdir/requirements.txt',
          packageFileName: 'pyproject.toml',
        },
    ...
    it('uvExportRequirements: skips export artifact when uv export command fails', async () => {
      const execSnapshots = mockExecSequence([
        { stdout: '', stderr: '' },
        new Error('uv export failed'),
      ]);
      GlobalConfig.set(adminConfig);
      mockLockAndRequirementsSetup();
      const requirementsWithHeader =
      ...
      expect(logger.logger.debug).toHaveBeenCalledWith(
        ...
        'uv export: failed to run uv export',
      );
    });
  });
});
