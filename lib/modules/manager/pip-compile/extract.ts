...
export async function extractAllPackageFiles(
  ...
  for (const packageFile of [...result].reverse()) {
    for (const reqFile of coerceArray<string>(
      ...
      // These get reversed before merging so that we keep the last instance of
      // any common lock files, since a file that -r includes multiple lock
      // files needs to be updated after all of the lock files it includes
