...
/**
 * Update pkgver and any version-aliasing variables in PKGBUILD
 * Version variables are custom underscore-prefixed variables (e.g., _pkgver, _basever)
 * whose value matches pkgver and are used in source URLs
 */
function updatePkgver(
  ...
  // Update all version-aliasing variables (e.g., _pkgver, _basever, _ver)
  if (versionVariables) {
    for (const varName of versionVariables) {
      const escapedVarName = escapeRegExp(varName);
      result = result.replace(
        regEx(`^(${escapedVarName}=["']?)${escapedOldVer}(["']?)$`, 'm'),
        `$1${newVer}$2`,
      );
    }
  }
...
/**
 * Expand version variables in URL
 * Replaces $pkgver, ${pkgver}, and any version-aliasing variables with the version value
 */
function expandUrl(
  url: string,
  pkgver: string,
  ...
  const ver = pkgver.replace(regEx(/^v/), '');
  let result = url
    .replace(regEx(/\$\{pkgver\}/g), ver)
    .replace(regEx(/\$pkgver/g), ver);

  // Expand all version-aliasing variables
  if (versionVariables) {
    for (const varName of versionVariables) {
      const escapedName = escapeRegExp(varName);
      result = result
        .replace(regEx(`\\$\\{${escapedName}\\}`, 'g'), ver)
        .replace(regEx(`\\$${escapedName}(?![a-zA-Z0-9_])`, 'g'), ver);
    }
  }
...
async function updateMultiSourceChecksums(
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    try {
      const downloadUrl = expandUrl(source.url, newValue, versionVariables);
      logger.debug(
...
function computeNewUrl(
  ...
  // If the URL contains pkgver variables, it doesn't need updating
  // (the variable update will take care of it)
  if (oldUrl.includes('$' + '{pkgver}') || oldUrl.includes('$pkgver')) {
    return oldUrl;
  }

  // Check if the URL references any version-aliasing variable
  if (versionVariables) {
    for (const varName of versionVariables) {
      if (
        oldUrl.includes('$' + `{${varName}}`) ||
        oldUrl.includes(`$${varName}`)
      ) {
        return oldUrl;
      }
...
export async function updateDependency({
  ...
  const { sourceUrl, checksums, pkgver, versionVariables, multiSource } =
    managerData;
  ...
  // Update pkgver and any version-aliasing variables
  ...
  // Handle single-source PKGBUILDs (backward compatibility)
    ...
    try {
      ...
      const downloadUrl = expandUrl(newUrl, newValue, versionVariables);
      const newChecksums = await computeChecksums(downloadUrl);
