...
function parseSourceUrl(url: string, pkgver: string): SourceData | null {
  try {
    ...
    const parsedUrl = parseUrl(expandedUrl);
    if (!parsedUrl) {
      return null;
    }
    ...
    // CPAN detection
    if (hostname === 'cpan.metacpan.org') {
      return parseCPANUrl(parsedUrl, expandedUrl);
    }

    // Packagist detection
    if (hostname === 'packagist.org' || hostname === 'repo.packagist.org') {
    ...
    // v8 ignore next -- defensive guard: parseUrl returns null and parser helpers never throw
    return null;
  }
}
