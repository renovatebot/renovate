...
const mockComputeChecksums = vi.mocked(computeChecksums);

// Deterministic fake checksums returned by the mock
const fakeChecksums = {
  sha256: 'f'.repeat(64),
  sha512: 'e'.repeat(128),
  b2: '9'.repeat(128),
...
describe('modules/manager/pkgbuild/update', () => {
  describe('updateDependency()', () => {
    ...
    it('updates pkgver and source URL', async () => {
      mockComputeChecksums.mockResolvedValueOnce(fakeChecksums);
...
`;
    ...
    it('continues without checksum update on download failure', async () => {
      mockComputeChecksums.mockRejectedValueOnce(new Error('download failed'));
...
`;
      ...
      // Two sources use pkgver, so computeChecksums is called twice
      mockComputeChecksums.mockResolvedValueOnce(fakeChecksums);
      mockComputeChecksums.mockResolvedValueOnce(fakeChecksums);
...
`;
      ...
      // Two sources use pkgver but file has only one checksum entry
      mockComputeChecksums.mockResolvedValueOnce(fakeChecksums);
      mockComputeChecksums.mockResolvedValueOnce(fakeChecksums);
...
`;
    ...
    it('updates _pkgver when listed in versionVariables', async () => {
      const content = `
pkgname=example-package
pkgver=1.2.3
_pkgver=1.2.3
...
`;
      ...
      const result = await updateDependency({
        ...
        upgrade: {
          ...
          newValue: 'v1.2.4',
          managerData: {
            sourceUrl: 'https://example.com/releases/example-${_pkgver}.tar.gz',
            checksums: {
              sha256:
                'abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890',
            },
            pkgver: '1.2.3',
            versionVariables: ['_pkgver'],
          },
        },
      });
      ...
      expect(result).not.toBeNull();
      expect(result).toContain('pkgver=1.2.4');
      expect(result).toContain('_pkgver=1.2.4');
      expect(result).toContain('pkgrel=1');
      expect(result).not.toContain('_pkgver=1.2.3');
    });

    it('updates custom version variable (e.g., _basever)', async () => {
      const content = `
pkgname=custom-pkg
pkgver=2.0.0
_basever=2.0.0
pkgrel=3
source=("https://example.com/downloads/\${_basever}/custom-pkg.tar.gz")
sha256sums=('abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890')
`;
      ...
      const result = await updateDependency({
        ...
        packageFile: 'PKGBUILD',
        upgrade: {
          depName: 'example/custom-pkg',
          currentValue: 'v2.0.0',
          newValue: 'v3.0.0',
          managerData: {
            sourceUrl:
              'https://example.com/downloads/${_basever}/custom-pkg.tar.gz',
            checksums: {
              sha256:
                'abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890',
            },
            pkgver: '2.0.0',
            versionVariables: ['_basever'],
          },
        },
      });

      expect(result).not.toBeNull();
      expect(result).toContain('pkgver=3.0.0');
      expect(result).toContain('_basever=3.0.0');
      expect(result).toContain('pkgrel=1');
      expect(result).not.toContain('_basever=2.0.0');
    });
...
`;
    ...
    it('replaces literal version when versionVariables are absent from the URL', async () => {
      const content = `
pkgname=literal-with-var
pkgver=1.0.0
_pkgver=1.0.0
pkgrel=1
source=("https://github.com/owner/repo/archive/v1.0.0.tar.gz")
...
`;
      ...
      const result = await updateDependency({
        ...
        upgrade: {
          ...
          managerData: {
            ...
            pkgver: '1.0.0',
            // version variable defined but not referenced in the source URL
            versionVariables: ['_pkgver'],
          },
        },
      });
      ...
      expect(result).not.toBeNull();
      expect(result).toContain('pkgver=2.0.0');
      // literal version in the URL is replaced directly
      expect(result).toContain('v2.0.0.tar.gz');
    });
