...
import { extractPackageFile } from './extract/index.ts';
import { extractAllSources } from './extract/utils.ts';
...
describe('modules/manager/pkgbuild/extract', () => {
  describe('extractPackageFile()', () => {
    ...
    it('extracts double-quoted checksums', () => {
      const content = codeBlock`
        pkgname=test-dquote
        pkgver=1.0.0
        source=("https://github.com/test/dquote/archive/v\${pkgver}.tar.gz")
        sha256sums=("abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890")
      `;
      const result = extractPackageFile(content);
    ...
    it('extracts double-quoted architecture-specific checksums', () => {
      const content = codeBlock`
        pkgname=test-dquote-arch
        pkgver=1.0.0
        arch=('x86_64' 'aarch64')
        source=("https://github.com/test/dquote-arch/archive/v\${pkgver}.tar.gz")
        sha256sums_x86_64=("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        sha256sums_aarch64=("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
      `;
      const result = extractPackageFile(content);
      expect(result?.deps[0].managerData?.checksums.sha256).toEqual([
        {
          value:
            'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
          suffix: '_x86_64',
    ...
    it('returns null when no source array found', () => {
      expect(extractPackageFile('pkgver=1.0.0\npkgname=foo')).toBeNull();
    });
    ...
    it('returns null for unsupported sources without pkgname', () => {
      const content = codeBlock`
        pkgver=1.0.0
    ...
    it('returns null for truly unsupported sources without pkgname', () => {
      const content = codeBlock`
        pkgver=1.0.0
    ...
    it('handles GitLab nested subgroups', () => {
      const content = codeBlock`
        pkgname=nested-gitlab
        pkgver=1.0.0
        source=("https://gitlab.com/owner/group/sub-group/repo/-/archive/v\${pkgver}/repo-v\${pkgver}.tar.gz")
        sha256sums=('1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef')
      `;
      const result = extractPackageFile(content);
      expect(result?.deps[0]).toMatchObject({
        depName: 'owner/group/sub-group/repo',
        currentValue: 'v1.0.0',
        datasource: 'gitlab-tags',
        managerData: {
          sourceUrl:
            'https://gitlab.com/owner/group/sub-group/repo/-/archive/v1.0.0/repo-v1.0.0.tar.gz',
        },
      });
    });
    ...
    it('does not match non-canonical cpan hostnames', () => {
      const content = codeBlock`
        pkgname=cpan-mirror
        pkgver=1.0.0
    ...
    it('returns null for filename-based source without pkgname', () => {
      const content = codeBlock`
        pkgver=1.0.0
    ...
    it('tracks multi-source where a version variable is absent from one source', () => {
      const content = codeBlock`
        pkgname=multi-novar
        pkgver=1.0.0
        _pkgver=1.0.0
        source=("https://github.com/owner/repo/archive/v\${pkgver}.tar.gz"
                "https://example.com/static/extra-data.tar.gz")
        sha256sums=('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
                    'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb')
      `;
      const result = extractPackageFile(content);
      const sources = result?.deps[0].managerData?.multiSource?.sources;
      // First source references pkgver directly
      expect(sources?.[0].usesPkgver).toBe(true);
      // Second source references neither pkgver nor any version variable
      expect(sources?.[1].usesPkgver).toBe(false);
    });
    ...
    it('returns null for array-style pkgname without supported source', () => {
      const content = codeBlock`
        pkgname=(foo bar baz)
        pkgver=1.0.0
        source=("https://example.com/pkg-1.0.0.tar.gz")
    ...
    it('returns null for GitLab URL where version is only an extension', () => {
      const content = codeBlock`
        pkgname=gl-ext-only
        pkgver=1.0.0
        source=("https://gitlab.com/owner/repo/-/archive/.tar.gz")
      `;
      const result = extractPackageFile(content);
      // version becomes empty after stripping extension → falls back to Repology
      expect(result?.deps[0]).toMatchObject({
        depName: 'aur/gl-ext-only',
        datasource: 'repology',
      });
    });
  });
  ...
  describe('extractAllSources()', () => {
    it('returns empty array when content has no source block', () => {
      const content = codeBlock`
        pkgname=no-source
        pkgver=1.0.0
        sha256sums=('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
      `;
      expect(extractAllSources(content)).toEqual([]);
    });
  });
});
