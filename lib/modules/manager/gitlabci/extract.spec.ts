...
describe('modules/manager/gitlabci/extract', () => {
  ...
  describe('extractAllPackageFiles()', () => {
    ...
    it('extracts images from services', async () => {
      const res = await extractAllPackageFiles(config, [
      ...
      expect(res).toEqual([
        {
          deps: [
            ...
            },
            {
              ...
              replaceString:
                'acme/app@sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855',
              autoReplaceStringTemplate:
                '{{depName}}{{#if newValue}}:{{newValue}}{{/if}}{{#if newDigest}}@{{newDigest}}{{/if}}',
              datasource: 'docker',
              depType: 'service-image',
            ...
            {
              depName: 'foo/postgresql',
              currentValue: '12.3-1',
              currentDigest: undefined,
              replaceString: 'foo/postgresql:12.3-1',
              autoReplaceStringTemplate:
                '{{depName}}{{#if newValue}}:{{newValue}}{{/if}}{{#if newDigest}}@{{newDigest}}{{/if}}',
              datasource: 'docker',
              depType: 'service-image',
            ...
            {
              depName: 'selenium/standalone-firefox',
              currentValue: 'latest',
              currentDigest: undefined,
              replaceString: 'selenium/standalone-firefox:latest',
              autoReplaceStringTemplate:
                '{{depName}}{{#if newValue}}:{{newValue}}{{/if}}{{#if newDigest}}@{{newDigest}}{{/if}}',
              datasource: 'docker',
              depType: 'service-image',
            ...
            {
              depName: 'registry.gitlab.com/organization/private-api',
              currentValue: 'latest',
              currentDigest: undefined,
              replaceString:
