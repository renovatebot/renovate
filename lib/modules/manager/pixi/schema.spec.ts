import { PixiConfigSchema, PoetrySources } from './schema';

describe('modules/manager/poetry/schema', () => {
  it('parses project version', () => {
    expect(
      PixiConfigSchema.parse({ version: '1.2.3' }).packageFileVersion,
    ).toBe('1.2.3');

    expect(
      PixiConfigSchema.parse({ version: { some: 'value' } }).packageFileVersion,
    ).toBeUndefined();
  });
