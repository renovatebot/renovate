...
Git credentials configured in `hostRules` are also automatically propagated to `hermit install` via `GIT_CONFIG_*` environment variables. This enables Hermit to fetch packages from private Git repositories without additional configuration.
