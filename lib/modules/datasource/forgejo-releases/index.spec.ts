...
import { ForgejoReleasesDatasource } from './index.ts';

const datasource = ForgejoReleasesDatasource.id;

describe('modules/datasource/forgejo-releases/index', () => {
  describe('getReleases', () => {
    it('returns tags from forgejo.com', async () => {
      const body = [
        {
          id: 266974,
          tag_name: 'v9.2.1',
      ...
      httpMock
        ...
        .get('/api/v1/repos/forgejo/helm-chart/releases?draft=false')
        .reply(200, body);
      const res = await getPkgReleases({
        datasource,
        packageName: 'forgejo/helm-chart',
      });
      expect(res).toEqual({
        ...
        sourceUrl: 'https://code.forgejo.org/forgejo/helm-chart',
      });
    });
  ...
  describe('getDigest with no commits', () => {
    it('returns commits from forgejo.com', async () => {
      httpMock
        .scope('https://code.forgejo.org')
        .get(
  ...
  describe('getTagCommit', () => {
    it('returns tags commit hash from forgejo.com', async () => {
      const body = {
        name: 'v9.0.1',
        message: 'v9.0.1',
      ...
      httpMock
        .scope('https://code.forgejo.org')
        .get('/api/v1/repos/forgejo/helm-chart/tags/v9.0.1')
        .reply(200, body);
      const res = await getDigest(
        {
          datasource,
