...
describe('modules/datasource/repology/index', () => {
  ...
  describe('queryPackage caching', () => {
    ...
    it.each([
      'https://repology.org',
      'https://repology.org/',
      'HTTPS://REPOLOGY.ORG:443/',
      'https://repology.org///',
      'https://repology.org/a/..',
      'https://repology.org/%2e/',
      'https://repology.org/a/%2E%2e/',
    ])('reuses public package metadata for %s', async (registryUrl) => {
      const packages = [
        { repo: 'debian_stable', visiblename: 'nginx', version: '1.14.2' },
      ];
      ...
      const first = await new RepologyDatasource().queryPackage(
        registryUrl,
        'debian_stable',
        'nginx',
      );
      memCache.reset();
      memCache.init();
      const second = await new RepologyDatasource().queryPackage(
        registryUrl,
        'debian_stable',
        'nginx',
      );

      expect(first).toEqual(packages);
      expect(second).toEqual(first);
    });

    it('preserves normal lookup errors for an invalid registry URL', async () => {
      await expect(
        new RepologyDatasource().queryPackage(
          'not-a-url',
          'some_repo',
          'example',
        ),
    ...
    it('reuses public metadata from the API fallback', async () => {
      const packages = [
        { repo: 'debian_stable', visiblename: 'nginx', version: '1.14.2' },
      ];
      mockResolverCall('debian_stable', 'nginx', 'binname', { status: 403 });
      mockApiCall('nginx', { status: 200, body: JSON.stringify(packages) });
      ...
      expect(first).toEqual(packages);
      expect(second).toEqual(packages);
    });

    it.each([
      'https://private.example',
      'http://repology.org',
      'https://repology.org:444',
      'https://repology.org.example',
      'https://www.repology.org',
      'https://repology.org/private',
      'https://repology.org/.git',
      'https://repology.org/%2f',
      'https://repology.org/?tenant=private',
      'https://repology.org/#private',
      'https://user:password@repology.org',
      'https://repology.org@private.example',
      'https://:password@repology.org',
    ])(
      'bypasses existing entries and does not store %s',
      async (registryUrl) => {
        const key = `cache-decorator:${joinUrlParts(registryUrl, 'some_repo', 'example')}`;
        const cached = [
          { repo: 'some_repo', version: '0.0.0', visiblename: 'example' },
        ];
        ...
        const first = [
          { repo: 'some_repo', version: '1.0.0', visiblename: 'example' },
        ];
        const second = [
          { repo: 'some_repo', version: '2.0.0', visiblename: 'example' },
        ];
        httpMock
          .scope(url.origin)
          .get(path)
        ...
        await expect(
          packageCache.get('datasource-repology', key),
        ).resolves.toEqual({
