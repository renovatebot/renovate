...
import { handleGenericErrors } from '../datasource.ts';
...
/**
 * The host type of requests to a Go module proxy. It is the id of the former
 * `go-proxy` datasource, so that `hostRules` for it keep matching.
 */
const goproxyHostType = 'go-proxy';

const http = new Http(goproxyHostType);

const githubHttp = new GithubHttp(GithubReleasesDatasource.id);
...
/**
 * go.getReleases, resolved through the module proxies configured with
 * `GOPROXY`, falling back to the module's source repository.
 *
 * The result is cached by `GoDatasource.getReleases()` under
 * {@link getReleasesCacheKey}, which is why nothing below caches releases of
 * its own.
 */
export async function getGoproxyReleases(
  ...
  if (goproxy === 'direct') {
    return getDirectReleases(config);
  }
  ...
  if (noproxy?.test(packageName)) {
    ...
    result = await getDirectReleases(config);
    return result;
  }
  ...
  for (const { url, fallback } of proxyList) {
    try {
      ...
      const res = await getVersionsWithInfo(
        url,
        packageName,
  ...
  if (result?.sourceUrl && servedByProxy) {
    await addGithubReleaseTimestamps(
      packageName,
      result.sourceUrl,
      result.releases,
    );
 ...
 */
async function addGithubReleaseTimestamps(
  packageName: string,
  sourceUrl: string,
  releases: Release[],
): Promise<void> {
...
export async function listVersions(
  ...
  const url = joinUrlParts(baseUrl, encodeCase(packageName), '@v', 'list');
...
export async function versionInfo(
  ...
  const url = joinUrlParts(
    baseUrl,
    encodeCase(packageName),
    '@v',
    `${version}.info`,
  );
  const res = await http.getJson(url, VersionInfo);
 ...
 */
function retrieveGoDirectiveForModule(
  baseUrl: string,
  packageName: string,
  version: string,
  ...
  return withCache(
    {
      // The namespace is that of the former `go-proxy` datasource, so that the
      // entries written by earlier Renovate versions stay valid.
      namespace: 'datasource-go-proxy',
      key: getVersionedCacheKey(packageName, version),
    ...
    () => fetchGoDirectiveForModule(baseUrl, packageName, version),
  );
}

async function fetchGoDirectiveForModule(
  baseUrl: string,
  packageName: string,
  version: string,
): Promise<string | undefined> {
...
async function getLatestVersion(
  ...
  try {
    const url = joinUrlParts(baseUrl, encodeCase(packageName), '@latest');
    const res = await http.getJson(url, VersionInfo);
...
async function getVersionsWithInfo(
  baseUrl: string,
  packageName: string,
  ...
  for (let major = packageMajor; ; major += 1) {
    ...
    try {
      const res = await listVersions(baseUrl, pkg);
      ...
      releases = await p.map(filteredReleases, async (info) => {
        const { version, newDigest, releaseTimestamp } = info;
        ...
        try {
          const release = await versionInfo(baseUrl, pkg, version);
      ...
      if (constraintsFiltering === 'strict') {
        releases = await p.map(releases, async (rel) => {
          try {
            const goDirective = await retrieveGoDirectiveForModule(
              baseUrl,
              pkg,
              rel.version,
            );
    ...
    const latest = await getLatestVersion(baseUrl, pkg);
    if (latest) {
...
/**
 * The package cache key of a `go` releases lookup.
 *
 * `GOPROXY` and `GONOPROXY` decide which host actually serves a module, so a
 * change to either must not be answered from a cache entry written under the
 * previous setting.
 */
export function getReleasesCacheKey({
  packageName,
...
function getVersionedCacheKey(packageName: string, version: string): string {
  const goproxy = getEnv().GOPROXY;
  const noproxy = parseNoproxy();
  // TODO: types (#22198)
