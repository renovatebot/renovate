...
The manager works the `registryUrl` out from the stage's base image, so any `atp` packages installed when using a `debian:trixie` base image will use the Debian 13 (Trixie) repositories.
Read the [`dockerfile` manager](../../manager/dockerfile/index.md) docs for the images it recognises.

If an image does not have a name that Renovate could derive the Alpine version from, for instance `node:22`, the `apt` packages will be skipped with `skipReason: unknown-registry` instead of performing an incorrect lookup.
