...
function autoExtendMavenRange(
  ...
  if (
    ...
    // if a range was detected where incrementing the lower value once results
    // in the upper value and the new version is outside the range, construct a
    // new range that follows the same semantic
    // [1,2) / 4.3.2 => [4,5)
    ...
    // same as last if, but properly considering Maven ordering specification
    // documented at
    ...
    // this is not correct as it matches various 2.x versions, mostly
    // pre-versions so the correct range definition would be [1,2-alpha) as
    // nothing starting with 2 can be lower than 2-alpha in Maven version
    // ordering logic
    //
    // so this if handles the conceptually same case as the last if just for the
    // case when proper Maven ranges are used
