Renovate's 'Same Major' versioning is designed to address scenarios where version specifications, denoted as `X.Y.Z`, signify a range of compatibility from greater than or equal to `X.Y.Z` to less than `X+1`.
