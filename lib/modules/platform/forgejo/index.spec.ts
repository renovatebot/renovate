...
describe('modules/platform/forgejo/index', () => {
  ...
  const mockPRs: MockPr[] = [
    partial<MockPr>({
      ...
      state: 'open',
      diff_url: 'https://forgejo.renovatebot.com/some/repo/pulls/1.diff',
    ...
    partial<MockPr>({
      ...
      state: 'closed',
      diff_url: 'https://forgejo.renovatebot.com/some/repo/pulls/2.diff',
    ...
    partial<MockPr>({
      ...
      state: 'closed',
      diff_url: 'https://forgejo.renovatebot.com/some/repo/pulls/4.diff',
      created_at: '2011-08-18T22:30:38Z',
      closed_at: '2016-01-09T10:03:21Z',
      updated_at: '2016-01-09T10:03:21Z',
      mergeable: true,
  ...
  async function initFakePlatform(
    ...
    scope
      ...
      .get('/version')
      .reply(200, { version });
    await forgejo.initPlatform({ token: 'abc' });
  }
  ...
  describe('initPlatform()', () => {
    ...
    it('should support custom endpoint', async () => {
      const scope = httpMock.scope('https://forgejo.renovatebot.com/api/v1');
      scope
        .get('/user')
        .reply(200, mockUser)
        .get('/version')
  ...
  describe('getRepos', () => {
    it('should propagate any other errors from getRepos', async () => {
      ...
      await expect(forgejo.getRepos()).rejects.toThrow('searchRepos()');
    });
    ...
    it('should return an array of repos', async () => {
      ...
      const repos = await forgejo.getRepos();
  ...
  describe('initRepo', () => {
    ...
    it('should abort when gitUrl has value ssh but ssh_url is empty', async () => {
      ...
      await expect(forgejo.initRepo(repoCfg)).rejects.toThrow(
        CONFIG_GIT_URL_UNAVAILABLE,
      );
    });
    ...
    it('should use given access token if gitUrl has value endpoint', async () => {
      ...
      hostRules.add({
        hostType: 'forgejo',
        matchHost: 'https://code.forgejo.org/',
        token,
      });
  ...
  describe('getBranchStatus', () => {
    ...
    it('should return yellow for unknown result', async () => {
      ...
      const res = await forgejo.getBranchStatus('some-branch', true);
    ...
    it('should not treat internal checks as success', async () => {
      ...
      const res = await forgejo.getBranchStatus('some-branch', false);
  ...
  describe('getBranchStatusCheck', () => {
    ...
    it('should return null with no matching results', async () => {
      ...
      const res = await forgejo.getBranchStatusCheck(
        'some-branch',
        'some-context',
      );
  ...
  describe('getPrList', () => {
    it('should return list of pull requests', async () => {
      ...
      const res = await forgejo.getPrList();
  ...
  describe('getPr', () => {
    it('should return enriched pull request which exists if open', async () => {
      ...
      const res = await forgejo.getPr(1);
    ...
    it('should return null for missing pull request in getPr', async () => {
      ...
      const res = await forgejo.getPr(42);
    ...
    it('should throw temporary error for null pull request', async () => {
      ...
      await expect(forgejo.getPr(42)).rejects.toThrow(TEMPORARY_ERROR);
    });
  });
  ...
  describe('findPr', () => {
    it('should find pull request without title or state', async () => {
      ...
      const res = await forgejo.findPr({ branchName: 'some-head-branch' });
    ...
    it('should find pull request with title', async () => {
      ...
      const res = await forgejo.findPr({
    ...
    it('should find merged pull request', async () => {
      ...
      const res = await forgejo.findPr({ branchName: 'merged-head-branch' });
    ...
    it('should return null for missing pull request in findPr', async () => {
      ...
      const res = await forgejo.findPr({ branchName: 'missing' });
  ...
  describe('createPr', () => {
    ...
    it('should ensure new pull request gets added to cached pull requests', async () => {
      ...
      const res = await forgejo.getPr(mockNewPR.number);
    ...
    it('should not use platform automerge on forgejo v7', async () => {
      ...
      const scope = httpMock
        ...
        .post('/repos/some/repo/pulls')
        .reply(200, mockNewPR);
      await initFakePlatform(scope, '7.0.0-dev-2136-f075579c95+forgejo-1.22.0');
      await initFakeRepo(scope);
    ...
    it('should not use platform automerge on forgejo v7 LTS', async () => {
      ...
      const scope = httpMock
        ...
        .post('/repos/some/repo/pulls')
        .reply(200, mockNewPR);
      await initFakePlatform(scope, '7.0.0+LTS-forgejo-1.22.0');
      await initFakeRepo(scope);
    ...
    it('continues if platform automerge is not supported', async () => {
      ...
      expect(logger.logger.debug).toHaveBeenCalledWith(
        ...
        'Forgejo-native automerge: skipped on unsupported version',
      );
    });
  ...
  describe('reattemptPlatformAutomerge', () => {
    ...
    it('does not warn when platform automerge is already scheduled', async () => {
      ...
      expect(logger.logger.debug).toHaveBeenCalledWith(
        { prNumber: 1 },
        'Forgejo-native automerge: already scheduled',
      );
    });
    ...
    it('logs a warning when enabling platform automerge fails', async () => {
      ...
      await expect(
        forgejo.reattemptPlatformAutomerge!({
          number: 1,
          platformPrOptions: { usePlatformAutomerge: true },
        }),
      ...
      expect(setTimeout).not.toHaveBeenCalled();
      expect(logger.logger.warn).toHaveBeenCalledWith(
        expect.objectContaining({ prNumber: 1, platform: 'forgejo' }),
        'Platform-native automerge: fail',
      );
    });
  });
  ...
  describe('updatePr', () => {
    it('should update pull request with title', async () => {
      ...
      await expect(
        forgejo.updatePr({ number: 1, prTitle: 'New Title' }),
      ).toResolve();
    });
  ...
  describe('mergePr', () => {
    it('should return true when merging succeeds', async () => {
      ...
      const res = await forgejo.mergePr({
        branchName: 'some-branch',
        id: 1,
      });
  ...
  describe('getIssueList', () => {
    it('should return empty for disabled issues', async () => {
      ...
      const res = await forgejo.getIssueList();
  ...
  describe('getIssue', () => {
    it('should return the issue', async () => {
      ...
      const res = await forgejo.getIssue?.(mockIssue.number);
  ...
  describe('findIssue', () => {
    it('should return existing open issue', async () => {
      ...
      const res = await forgejo.findIssue(mockIssue.title);
    ...
    it('should return null for missing issue', async () => {
      ...
      const res = await forgejo.findIssue('missing');
  ...
  describe('ensureIssue', () => {
    it('should create issue if not found', async () => {
      ...
      const res = await forgejo.ensureIssue(mockIssue);
  ...
  describe('ensureIssueClosing', () => {
    it('should close issues with matching title', async () => {
      ...
      await expect(forgejo.ensureIssueClosing(mockIssue.title)).toResolve();
    });
    ...
    it('should return for disabled issues', async () => {
      ...
      await initFakePlatform(scope);
      await initFakeRepo(scope, { has_issues: false });
      await expect(forgejo.ensureIssueClosing('new-title')).toResolve();
    });
  });
  ...
  describe('deleteLabel', () => {
    it('should delete a label which exists', async () => {
      ...
      await expect(forgejo.deleteLabel(42, mockLabel.name)).toResolve();
    });
  ...
  describe('ensureComment', () => {
    it('should add comment with topic if not found', async () => {
      ...
      const res = await forgejo.ensureComment({
        number: 1,
        topic: 'other-topic',
  ...
  describe('ensureCommentRemoval', () => {
    ...
    it('should gracefully fail with warning when removing comment', async () => {
      ...
      await forgejo.ensureCommentRemoval({
        type: 'by-topic',
        number: 1,
        topic: 'some-topic',
      });
  ...
  describe('getBranchPr', () => {
    it('should return existing pull request for branch', async () => {
      ...
      const res = await forgejo.getBranchPr('some-head-branch');
  ...
  describe('addAssignees', () => {
    it('should add assignees to the issue', async () => {
      ...
      await expect(forgejo.addAssignees(1, ['me', 'you'])).toResolve();
    });
  });
