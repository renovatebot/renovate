...
function httpLink(
  ...
  return `${ensureTrailingSlash(endpointStr)}scm/${projectKey.toLowerCase()}/${repositorySlug}.git`;
}
...
<!-- followed by some more comments -->`);
    ...
    describe('modules/platform/bitbucket-server/code-owners', () => {
      ...
      it('parses usernames with escaped spaces', () => {
        ...
        expect(rules[0].usernames).toEqual(['@Jane Doe', '@john@example.com']);
      });
      ...
      it('supports reviewer groups with modifiers)', () => {
        ...
        expect(rules[0].usernames).toEqual(['@reviewer-group/devs:random(2)']);
  ...
  describe('endpoint with no path', () => {
    ...
    describe('initRepo()', () => {
      it('gitURL endpoint generates URL without endpoint path', async () => {
        httpMock
          .scope(urlHost)
          .get(`${urlPath}/rest/api/1.0/projects/SOME/repos/repo`)
          .reply(200, repoMock(url, 'SOME', 'repo'))
        ...
        git.getUrl.mockReturnValueOnce('https://generated-url/repo.git');
        ...
        expect(git.getUrl).toHaveBeenCalledWith(
          expect.objectContaining({
            host: 'stash.renovatebot.com/scm',
          }),
        );
      });

      it('generates URL without endpoint path if API does not contain clone links', async () => {
        httpMock
          .scope(urlHost)
          .get(`${urlPath}/rest/api/1.0/projects/SOME/repos/repo`)
          .reply(
