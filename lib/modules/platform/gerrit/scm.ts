...
/**
 * Build the Gerrit push options for a `refs/for/` push.
 */
function buildGerritPushOptions(options?: {
  autoApprove?: boolean;
  labels?: string[];
}): string[] {
...
export async function pushForReview(options: {
  ...
  const pushOptions = buildGerritPushOptions(options);
...
export class GerritScm extends DefaultGitScm {
  override async commitAndPush(
    ...
    if (existingChange) {
      ...
      const commitSha = await git.commitFiles({
        ...commit,
        targetRef: `refs/for/${existingChange.branch}`,
        pushOptions: buildGerritPushOptions({
          autoApprove: commit.autoApprove,
        }),
      });
      if (commitSha) {
        // When pushing to a custom ref (e.g. refs/for/), git does not
        // auto-update the remote-tracking ref, so update it manually.
