...
describe('modules/platform/azure/index', () => {
  ...
  describe('initRepo URL encoding', () => {
    it('encodes whitespace in project when falling back to manualUrl', async () => {
      azureApi.gitApi.mockResolvedValueOnce(
        partial<IGitApi>({
          getRepositories: vi.fn().mockResolvedValue([
            {
      ...
      expect(git.initRepo).toHaveBeenCalledWith(
        expect.objectContaining({
          url: 'https://dev.azure.com/renovate12345/my%20project/_git/my-repo',
        }),
      );
    });

    it('encodes whitespace in repo when falling back to manualUrl', async () => {
      azureApi.gitApi.mockResolvedValueOnce(
        partial<IGitApi>({
          getRepositories: vi.fn().mockResolvedValue([
            {
      ...
      expect(git.initRepo).toHaveBeenCalledWith(
        expect.objectContaining({
          url: 'https://dev.azure.com/renovate12345/my-project/_git/my%20repo',
        }),
      );
    });
    ...
    it('encodes whitespace in project when using repo.remoteUrl', async () => {
      azureApi.gitApi.mockResolvedValueOnce(
        partial<IGitApi>({
          getRepositories: vi.fn().mockResolvedValue([
            {
      ...
      expect(git.initRepo).toHaveBeenCalledWith(
        expect.objectContaining({
          url: 'https://dev.azure.com/my-org/my%20project/_git/my-repo',
        }),
      );
    });

    it('encodes whitespace in repo when using repo.remoteUrl', async () => {
      azureApi.gitApi.mockResolvedValueOnce(
        partial<IGitApi>({
          getRepositories: vi.fn().mockResolvedValue([
            {
      ...
      expect(git.initRepo).toHaveBeenCalledWith(
        expect.objectContaining({
          url: 'https://dev.azure.com/my-org/my-project/_git/my%20repo',
        }),
      );
    });
    ...
    it('still encodes special characters in project/repo names when using manualUrl', async () => {
      azureApi.gitApi.mockResolvedValueOnce(
        partial<IGitApi>({
          getRepositories: vi.fn().mockResolvedValue([
            {
      ...
      await azure.initRepo({ repository: 'my & project/repo' });

      expect(git.initRepo).toHaveBeenCalledWith(
        expect.objectContaining({
          url: 'https://dev.azure.com/renovate12345/my%20%26%20project/_git/repo',
        }),
      );
    });

    it('produces an unchanged URL for a plain project/repo name via manualUrl (regression baseline)', async () => {
      azureApi.gitApi.mockResolvedValueOnce(
        partial<IGitApi>({
          getRepositories: vi.fn().mockResolvedValue([
            {
      ...
      await azure.initRepo({ repository: 'some/repo' });

      expect(git.initRepo).toHaveBeenCalledWith(
        expect.objectContaining({
          url: 'https://dev.azure.com/renovate12345/some/_git/repo',
        }),
      );
    });

    it('produces an unchanged URL for a plain project/repo name via repo.remoteUrl (regression baseline)', async () => {
      azureApi.gitApi.mockResolvedValueOnce(
        partial<IGitApi>({
          getRepositories: vi.fn().mockResolvedValue([
            {
              ...
              defaultBranch: 'main',
              project: { name: 'some' },
              remoteUrl: 'https://dev.azure.com/renovate12345/some/_git/repo',
            },
          ]),
        }),
      );
    ...
    it('preserves an on-prem Azure DevOps Server collection path via manualUrl', async () => {
      await azure.initPlatform({
        endpoint:
          'https://azure-devops.internal.corp:8080/tfs/DefaultCollection/',
        token: 'token',
      });
      ...
      expect(git.initRepo).toHaveBeenCalledWith(
        expect.objectContaining({
          url: 'https://azure-devops.internal.corp:8080/tfs/DefaultCollection/my-project/_git/my-repo',
        }),
      );
    });

    it('does not throw when project name contains a literal "%" character (manualUrl fallback)', async () => {
      // A project/repo name may contain a literal '%' that isn't part of a
      // valid percent-encoded sequence (e.g. "50% off"). This must not throw
      // and must behave like encodeURIComponent() would have on the raw name.
      azureApi.gitApi.mockResolvedValueOnce(
        partial<IGitApi>({
          getRepositories: vi.fn().mockResolvedValue([
            {
      ...
      await azure.initRepo({ repository: '50% off/repo' });

      expect(git.initRepo).toHaveBeenCalledWith(
        expect.objectContaining({
          url: 'https://dev.azure.com/renovate12345/50%25%20off/_git/repo',
        }),
      );
    });
  });
