...
Set the token and auth type via config or environment variables:
...
export RENOVATE_AZURE_AUTH_TYPE=bearer
```

The `azureAuthType` option accepts:

| Value      | Description                                                                 |
| ---------- | --------------------------------------------------------------------------- |
| `"auto"`   | (Default) Detects token type by format. 52-char tokens are treated as PATs. |
| `"pat"`    | Force Personal Access Token authentication.                                 |
| `"bearer"` | Force Bearer token (Microsoft Entra ID) authentication.                     |
