...
export class SpaceCodeReviewReadClient {
  ...
  async getByCodeReviewId(
    ...
    const result = await this.http.getJson<SpaceMergeRequestRecord>(
      `/api/http/projects/key:${projectKey}/code-reviews/id:${codeReviewId}`,
    );
    return result.body;
  }

  async getByCodeReviewNumber(
    projectKey: string,
    codeReviewNumber: number,
  ): Promise<SpaceMergeRequestRecord> {
    const result = await this.http.getJson<SpaceMergeRequestRecord>(
      `/api/http/projects/key:${projectKey}/code-reviews/number:${codeReviewNumber}`,
    );
    return result.body;
  }
  ...
  async find(
    ...
    return await mapNotNullFlatten(
      ...
      async (basicReview) => {
        ...
        const review = await this.getByCodeReviewId(
          projectKey,
          basicReview.review.id,
        );
        if (review.state === 'Deleted') {
          // should not normally be returned, but who knows
          logger.info(
            `SPACE: Ignoring PR ${review.title} because it is deleted`,
          );
          return undefined;
        }
  ...
  async getMessages(
    ...
    const apiOrder =
      order === 'asc' ? 'FromOldestToNewest' : 'FromNewestToOldest';

    const iterable = PaginatedIterable.fromUsing<SpaceChannelItem>(
      this.http,
      `/api/http/chats/messages?channel=codeReview:id:${codeReviewId}&sorting=${apiOrder}&batchSize=${limit}`,
      {
