...
describe('modules/platform/space/client/jobs', () => {
  ...
  describe('getAllJobs()', () => {
    it('should return jobs', async () => {
      ...
      expect(await client.getAllJobs(projectKey, repository, branch)).toEqual([job1, job2]);
    });
  });
  ...
  describe('findJobExecutions()', () => {
    it('should find job executions', async () => {
      ...
      mockJobExecutionsPage(projectKey, jobId, branch, [execution1, filteredOutExecution], next1)
      mockJobExecutionsPage(projectKey, jobId, branch, [execution2], next2, next1)
      mockJobExecutionsPage(projectKey, jobId, branch, [], next2, next2)

      const actual = await client.findJobExecutions(projectKey, jobId, branch, it => it.commitId !== filteredOutCommit)
      expect(actual).toEqual([execution1, execution2])
    })
  })
});

function mockJobsPage(projectKey: string, repository: string, branch: string, jobs: SpaceJobDTO[], next: string, nextQuery?: string) {
...
function mockJobExecutionsPage(projectKey: string, jobId: string, branch: string, executions: SpaceJobExecutionDTO[], next: string, nextQuery?: string) {
