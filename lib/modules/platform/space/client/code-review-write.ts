...
import type {CodeReviewStateFilter, SpaceCodeReviewCreateRequest, SpaceCodeReviewParticipantRole, SpaceMergeRequestRecord} from "../types";
...
export class SpaceCodeReviewWriteClient {
  ...
  async create(projectKey: string, request: SpaceCodeReviewCreateRequest): Promise<SpaceMergeRequestRecord> {
    logger.debug(`SPACE: createMergeRequest: projectKey=${projectKey}, request=${JSON.stringify(request)}`)

    const response = await this.http.postJson<SpaceMergeRequestRecord>(`/api/http/projects/key:${projectKey}/code-reviews/merge-requests`, {body: request})
    logger.debug(`SPACE: createMergeRequest: response: ${JSON.stringify(response.body)}`)

    return response.body
  }

  async merge(projectKey: string, codeReviewNumber: number, mergeMode: 'FF' | 'FF_ONLY' | 'NO_FF', deleteSourceBranch: boolean): Promise<void> {
  ...
  async rebase(projectKey: string, codeReviewNumber: number, rebaseMode: 'FF' | 'NO_FF', squashedCommitMessage: string | null, deleteSourceBranch: boolean): Promise<void> {
  ...
  async addMessage(codeReviewId: string, comment: string, externalId: string | null): Promise<void> {
  ...
  async setTitle(projectKey: string, codeReviewId: string, title: string): Promise<void> {
    await this.http.patchJson(`/api/http/projects/key:${projectKey}/code-reviews/id:${codeReviewId}/title`, {body: {title}})
  }

  async setDescription(projectKey: string, codeReviewId: string, description: string): Promise<void> {
    logger.debug(`SPACE: updateCodeReviewDescription: projectKey=${projectKey}, codeReviewId=${codeReviewId}, description=${description}`)
    await this.http.patchJson(`/api/http/projects/key:${projectKey}/code-reviews/id:${codeReviewId}/description`, {body: {description}})
  }

  async setState(projectKey: string, codeReviewId: string, state: CodeReviewStateFilter): Promise<void> {
    logger.debug(`SPACE: updateCodeReviewState: projectKey=${projectKey}, codeReviewId=${codeReviewId}, state=${state}`)
    await this.http.patchJson(`/api/http/projects/key:${projectKey}/code-reviews/id:${codeReviewId}/state`, {body: {state}})
  }
}
