...
async function checkForClosedPr(branchName, prTitle) {
  ...
  const res = await vstsGot()
    .getGitApi()
    .getPullRequests(config.repoId, { status: 4 });
  const prFound = _.filter(res, pr => pr.title === prTitle);
  return prFound.length !== 0;
}
...
async function commitFilesToBranch(
  ...
  let newBranch = helper.getVSTSBranchObj(
    branchName,
    (await getRef(parentBranch))[0].objectId
  );
  ...
  for (const file of files) {
    ...
    if (isBranchExisting) {
      newBranch = helper.getVSTSBranchObj(
        branchName,
        (await getRef(`refs/heads/${branchName}`))[0].objectId
      );
    }

    const commit = helper.getVSTSCommitObj(message, file.name, file.contents);
