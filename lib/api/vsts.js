...
async function branchExists(branchName) {
  throw new Error(`vsts error: branchExists(${branchName})`);
  ...
  //     // This seems to happen if vsts has partial matches, so we check ref
...
async function getBranchPr(branchName) {
  throw new Error(`vsts error: getBranchPr(${branchName})`);
...
async function createPr(branchName, title, body, useDefaultBranch) {
  throw new Error(
    `vsts error: createPr(${branchName}, ${title}, ${body}, ${useDefaultBranch})`
  );
  ...
  try {
    const item = await vstsGot()
      ...
      .getItemText(
        ...
        {
          versionType: 0,
          version: getBranchNameFromPath(branchName),
        }
      );
...
async function commitFilesToBranch(
  ...
  const parentTree = await getCommitTree(parentBranch);
  const fileBlobs = [];
  // Create blobs
...
function getBranchNameFromPath(branchPath) {
  const lBranchPath = branchPath.split('/');
  return lBranchPath[lBranchPath.length - 1];
}
...
async function getBranchCommit(branchName) {
  const commit = await vstsGot()
    ...
    .getBranch(
      config.repoId,
      getBranchNameFromPath(branchName),
      config.repoName
    );
