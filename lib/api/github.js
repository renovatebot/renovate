...
async function getInstallations(appToken) {
  ...
  try {
    const res = await ghGot('app/installations', {
      headers: {
    		'accept': 'application/vnd.github.machine-man-preview+json',
    		'authorization': `Bearer ${appToken}`
    	}
    });
    logger.verbose(`Returning ${res.body.length} results`);
    return res.body;
  } catch (err) {
...
async function getInstallationToken(appToken, installationId) {
  ...
  try {
    // https://api.github.com/installations/30174/access_tokens
    const res = await ghGot.post(`installations/${installationId}/access_tokens`, {
      headers: {
    		'accept': 'application/vnd.github.machine-man-preview+json',
    		'authorization': `Bearer ${appToken}`
    	}
...
async function getInstallationRepositories(userToken) {
  ...
  try {
    const res = await ghGot('installation/repositories', {
      headers: {
    		'accept': 'application/vnd.github.machine-man-preview+json',
    		'authorization': `token ${userToken}`
    	}
    });
    logger.verbose(`Returning ${res.body.repositories.length} results from a total of ${res.body.total_count}`);
    return res.body;
  } catch (err) {
