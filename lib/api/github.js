...
function initRepo(repoName) {
  ...
  .then(getRepoBaseCommitSHA)
  .then(getRepoBaseTreeSHA)
  .catch((err) => {
  ...
  function getRepoBaseCommitSHA() {
    logger.debug(`Getting ${config.defaultBranch} Commit SHA for ${repoName}`);
    return ghGot(`repos/${config.repoName}/git/refs/head`).then((res) => {
      ...
      res.body.forEach((branch) => {
        ...
        if (branch.ref === `refs/heads/${config.defaultBranch}`) {
          ...
          config.baseCommitSHA = branch.object.sha;
          logger.debug(`${config.defaultBranch} Commit SHA = ${branch.object.sha}`);
        }
      });
      return Promise.resolve();
    });
  ...
  function getRepoBaseTreeSHA() {
    logger.debug(`Getting ${config.defaultBranch} Tree SHA for ${repoName}`);
    return ghGot(`repos/${config.repoName}/git/commits/${config.baseCommitSHA}`).then((res) => {
      // Get the SHA for base tree
      config.baseTreeSHA = res.body.tree.sha;
      logger.debug(`${config.defaultBranch} Tree SHA = ${res.body.tree.sha}`);
      return Promise.resolve();
    });
  }
}
