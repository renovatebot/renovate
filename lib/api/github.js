...
// Creates branch if doesn't already exist, otherwise updates commit sha for existing branch
function setBranchCommit(branchName, commit) {
  return branchExists(branchName)
  .then((exists) => {
    if (exists) {
      logger.debug(`Branch ${branchName} exists - updating it to new commit ${commit}`);
      return updateBranch(branchName, commit);
    }
    logger.debug(`Creating new branch ${branchName} for commit ${commit}`);
...
function commitFile(parentBranch, fileName, fileContents, message) {
