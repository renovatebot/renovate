...
async function getRateLimit() {
  const res = await ghGot('rate_limit');
  return res.body;
}
...
async function createPr(branchName, title, body) {
  const res = await ghGot.post(`repos/${config.repoName}/pulls`, {
...
async function getPr(prNo) {
  const res = await ghGot(`repos/${config.repoName}/pulls/${prNo}`);
  return res.body;
}
...
async function getFileContent(filePath, branchName = config.baseBranch) {
  try {
    const res = await getFile(filePath, branchName);
    return new Buffer(res.body.content, 'base64').toString();
  } catch (error) {
    if (error.statusCode === 404) {
...
async function createBlob(fileContents) {
  ...
  const res = await ghGot.post(`repos/${config.repoName}/git/blobs`, {
    body: {
      encoding: 'base64',
      content: new Buffer(fileContents).toString('base64'),
    },
...
async function getCommitTree(commit) {
  ...
  const res = await ghGot(`repos/${config.repoName}/git/commits/${commit}`);
  return res.body.tree.sha;
}
...
async function createTree(baseTree, filePath, fileBlob) {
  ...
  const res = await ghGot.post(`repos/${config.repoName}/git/trees`, {
    body: {
      base_tree: baseTree,
      tree: [
        {
...
async function createCommit(parent, tree, message) {
  ...
  const res = await ghGot.post(`repos/${config.repoName}/git/commits`, {
    body: {
      message,
      parents: [parent],
      tree,
