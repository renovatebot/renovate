...
async function initRepo(repoName, token, endpoint) {
  ...
  try {
    ...
    config.canRebaseMerge = res.body.allow_rebase_merge;
    config.canSquashMerge = res.body.allow_squash_merge;
...
async function mergePr(prNo) {
  // Autodetect best merge type
  let mergeMethod = 'merge';
  if (config.canRebaseMerge) {
    mergeMethod = 'rebase';
  } else if (config.canSquashMerge) {
    mergeMethod = 'squash';
  }
  await ghGot.put(`repos/${config.repoName}/pulls/${prNo}/merge`, {
    body: {
