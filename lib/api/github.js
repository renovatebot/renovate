...
async function getDirectoryContents(branchName = config.baseBranch, path = '') {
  const url = `repos/${config.repoName}/contents/${path}?ref=${branchName}`;
  logger.debug({ url }, 'getFileList');
  ...
  const subdirectoryFiles = (await Promise.all(
    directories.map(dir => getDirectoryContents(branchName, dir))
  ))
...
async function getFileList(branchName = config.baseBranch) {
  if (!config.fileList) {
    const res = await ghGotRetry(
      `repos/${config.repoName}/git/trees/${branchName}?recursive=1`
    );
    if (res.body.truncated) {
      logger.warn({ repository: config.repoName }, 'getTree is truncated');
      config.fileList = await getDirectoryContents(branchName);
    } else {
...
async function findFilePaths(fileName, branchName = config.baseBranch) {
  return (await getFileList(branchName)).filter(f => f.endsWith(fileName));
}
...
async function addReviewers(issueNo, reviewers) {
  ...
  const res = await ghGotRetry.post(
