...
function findPr(branchName, prTitle, state) {
  logger.debug(`findPr(${branchName}, ${prTitle}, ${state})`);
  let urlString = `repos/${config.repoName}/pulls?head=${config.owner}:${branchName}`;
  if (state) {
    urlString = `${urlString}&state=${state}`;
  }
  logger.debug(`findPr urlString: ${urlString}`);
  return ghGot(urlString)
  .then((res) => {
    if (!res.body.length) {
      logger.debug('findPr: not found');
      return false;
    }
    if (prTitle) {
      return res.body[0].title === prTitle;
    }
    return true;
  });
}
