...
async function processRepo(repo) {
  ...
  // Set token for this repo
  if (repo.platform === 'github') {
    process.env.GITHUB_TOKEN = process.env.GITHUB_TOKEN || config.token;
    if (process.env.GITHUB_TOKEN === undefined) {
      logger.error(`No token found for GitHub repository ${repo.repository}`);
      return;
    }
  }
  if (repo.platform === 'gitlab') {
    process.env.GITLAB_TOKEN = process.env.GITLAB_TOKEN || config.token;
    if (process.env.GITLAB_TOKEN === undefined) {
      logger.error(`No token found for GitLab repository ${repo.repository}`);
      return;
    }
  }
  try {
    // Initialize repo
    await api.initRepo(config.repository);
...
async function checkIfConfigured(config) {
  ...
  const newPrNo = await configureRepository();
  logger.info(`Created PR #${newPrNo} for configuration`);
  return false;
}
...
If the default settings are all suitable for you, simply close this Pull Request unmerged and your first renovation will begin the next time the program is run.`;
  ...
  await api.commitFileToBranch('renovate/configure', 'renovate.json', defaultConfigString, 'Add renovate.json');
  const prNo = await api.createPr('renovate/configure', 'Configure Renovate', prBody);
  return prNo;
}
