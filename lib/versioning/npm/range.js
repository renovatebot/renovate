...
function cappedWithNewValue(currentValue, toVersion) {
  ...
  let currentParsed;
  let compactParsed;
  const compactResult = `${currentValue} <=${toVersion}`;
  try {
    currentParsed = new Range(currentValue);
    compactParsed = new Range(currentValue);
  } catch (e) {
    return toVersion;
  }
  let skip = false;
    ...
    parsed.set
      .reduce((accum, comparators) => {
        ...
        comparators.forEach(comparator => {
          ...
          if (!op) {
            skip = true;
          } else if (op.startsWith('<') && compare(version, toVersion) !== 1) {
            skip = true;
          } else {
            items.push(`${op}${version}`);
          }
        });
        return [...accum, [...items, `<=${toVersion}`].join(' ')];
      }, [])
      .join(' || ');
  const fullResult = extend(currentParsed);
  if (skip) return fullResult;
  return fullResult === extend(compactParsed) ? compactResult : fullResult;
}
