...
export const matchVersion = regEx(
  `^(?P<version>${versionGroup})$`
); /* Match a version number (e.g. 1.0.0) */
export const exactVersion = regEx(
  `^(?P<exact_version>==(?P<exact_version_group>${versionGroup})?)$`
...
export const inclusiveBound = regEx(
  `^(?P<inclusive_bound>(?P<inclusive_lower_version>${versionGroup})?\\.\\.(?P<inclusive_upper_version>${versionGroup})?)$`
...
export const upperBound = new RegExp( // TODO #12070 backreference not supported in RE2
