...
describe('versioning/maven/index', () => {
  ...
  it('returns valid', () => {
    ...
    expect(isValid('[1.12.6,1.18.6]')).toBeTrue();
    expect(isValid(undefined)).toBeFalse();
    expect(isValid === _isValid).toBeTrue();
  });
  it('validates version string', () => {
    ...
    expect(isVersion('1-foo')).toBeTrue();
    ...
    expect(isVersion('-1')).toBeFalse();
    expect(isVersion('1-')).toBeFalse();
    expect(isVersion('[1.12.6,1.18.6]')).toBeFalse();
    expect(isVersion('RELEASE')).toBeFalse();
    expect(isVersion('release')).toBeFalse();
    expect(isVersion('LATEST')).toBeFalse();
    expect(isVersion('latest')).toBeFalse();
    expect(isVersion('foobar')).toBeTrue();
  });
  it('checks if version is stable', () => {
    expect(isStable('')).toBeNull();
    expect(isStable('foobar')).toBeTrue();
    expect(isStable('final')).toBeTrue();
    expect(isStable('1')).toBeTrue();
    expect(isStable('1.2')).toBeTrue();
    expect(isStable('1.2.3')).toBeTrue();
    expect(isStable('1.2.3.4')).toBeTrue();
    expect(isStable('v1.2.3.4')).toBeTrue();
    expect(isStable('1-alpha-1')).toBeFalse();
    expect(isStable('1-b1')).toBeFalse();
    expect(isStable('1-foo')).toBeTrue();
    expect(isStable('1-final-1.0.0')).toBeTrue();
    expect(isStable('1-release')).toBeTrue();
    expect(isStable('1.final')).toBeTrue();
    expect(isStable('1.0milestone1')).toBeFalse();
    expect(isStable('1-sp')).toBeTrue();
    expect(isStable('1-ga-1')).toBeTrue();
    expect(isStable('1.3-groovy-2.5')).toBeTrue();
    expect(isStable('1.3-RC1-groovy-2.5')).toBeFalse();
    expect(isStable('Hoxton.RELEASE')).toBeTrue();
    expect(isStable('Hoxton.SR')).toBeTrue();
    expect(isStable('Hoxton.SR1')).toBeTrue();
  });
  ...
  it('matches against maven ranges', () => {
    ...
    expect(matches('1', '[1,)')).toBeTrue();
    expect(matches('1', '(1,)')).toBeFalse();
    expect(matches('1', '(,1),(1,)')).toBeFalse();
    expect(matches('1', '(0,1),(1,2)')).toBeFalse();
    expect(matches('1.0.0.RC9.2', '(,1.0.0.RC9.2),(1.0.0.RC9.2,)')).toBeFalse();
    expect(matches('1.0.0-RC14', '(,1.0.0.RC9.2),(1.0.0.RC9.2,)')).toBeTrue();
  ...
  it('api', () => {
    expect(maven.isGreaterThan('1.1', '1')).toBeTrue();
