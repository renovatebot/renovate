...
describe('config/validation', () => {
  describe('validateConfig(config)', () => {
    ...
    it('errors for unsafe filePatterns', async () => {
      const config = {
        npm: {
          filePatterns: ['/abc ([a-z]+) ([a-z]+))/'],
        },
        dockerfile: {
          filePatterns: ['/x?+/'],
        },
    ...
    it('validates regEx for each filePatterns of format regex', async () => {
      const config: RenovateConfig = {
        customManagers: [
          {
            customType: 'regex',
    ...
    it('errors if customManager has empty filePatterns', async () => {
      const config = {
        customManagers: [
          {
            customType: 'regex',
    ...
    it('errors if no customManager filePatterns', async () => {
      const config = {
        customManagers: [
          {
            matchStrings: ['^(?<depName>foo)(?<currentValue>bar)$'],
    ...
    // testing if we get all errors at once or not (possible), this does not include customType or filePatterns
    ...
    it('errors if filePatterns has wrong parent', async () => {
      const config: RenovateConfig = {
        filePatterns: ['foo'],
        npm: {
          filePatterns: ['package\\.json'],
      ...
      it('onboardingConfig', async () => {
        const config = {
          onboardingConfig: {
            ...
            filePatterns: ['somefile'], // invalid at top level
          },
        };
        const { warnings } = await configValidation.validateConfig(
          'global',
        ...
        expect(warnings).toEqual([
          {
            message:
              '"filePatterns" may not be defined at the top level of a config and must instead be within a manager block',
            topic: 'Config error',
          },
          {
            topic: 'Configuration Error',
