...
function mergeChildConfig(parentConfig, childConfig) {
  ...
  for (const option of definitions.getOptions()) {
    if (option.mergeable && childConfig[option.name]) {
      ...
      if (option.name === 'depTypes') {
        const depTypes = [];
        for (let parent of parentConfig.depTypes) {
          if (typeof parent === 'string') {
            parent = { depType: parent };
          }
          for (let child of childConfig.depTypes) {
            if (typeof child === 'string') {
              child = { depType: child };
            }
            if (parent.depType === child.depType) {
              Object.assign(parent, child);
            }
          }
          depTypes.push(parent);
        }
        config.depTypes = depTypes;
      } else {
