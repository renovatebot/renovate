...
async function parseConfigs(env, argv) {
  ...
  if (!Object.keys(defaultEndpoints).includes(config.platform)) {
    throw new Error(`Unsupported platform: ${config.platform}`);
  }

  config.endpoint = config.endpoint || defaultEndpoints[config.platform];

  if (!config.endpoint) {
    throw new Error('Must configure an endpoint');
  }
