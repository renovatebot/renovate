...
async function parseConfigs(env, argv) {
  ...
  if (!config.endpoint) {
    if (config.platform === 'github') {
      ...
      config.endpoint =
        process.env.GITLAB_ENDPOINT || 'https://gitlab.com/api/v4/';
    } else if (config.platform === 'bitbucket') {
      config.endpoint =
        process.env.BITBUCKET_ENDPOINT || 'https://api.bitbucket.org/';
    } else if (config.platform === 'vsts') {
      config.endpoint = process.env.VSTS_ENDPOINT;
      if (!config.endpoint) {
        throw new Error('Must configure VSTS_ENDPOINT');
      }
    } else {
  ...
  const { token } = hostRules.find(config);

  if (!token) {
    throw new Error(
      `Missing token for ${config.platform} endpoint ${config.endpoint}`
    );
  }
