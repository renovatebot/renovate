...
function getDepTypeConfig(packageFileConfig, index) {
  let depType = packageFileConfig.depTypes[index];
  if (typeof depType === 'string') {
    depType = { depType };
  }
  const depTypeConfig = Object.assign({}, packageFileConfig, depType);
  for (const option of definitions.getOptions()) {
    if (option.level === 'packageFile') {
      delete depTypeConfig[option.name];
    }
  }
...
function getDepConfig(depTypeConfig, dep) {
  ...
  for (const option of definitions.getOptions()) {
    if (option.level === 'depType') {
      delete depConfig[option.name];
    }
  }
