...
async function parseConfigs(env, argv) {
  ...
  let credentials = endpoints.find({ platform }, {});

  if (!endpoint && credentials.endpoint) {
    endpoints.update({ platform, ...credentials, token });
    credentials = endpoints.find({ platform }, {});
  }

  if (!credentials.endpoint) {
    throw new Error(`You need to supply a ${platformName} endpoint.`);
  }

  if (!credentials.token) {
    throw new Error(`You need to supply a ${platformName} token.`);
  }
