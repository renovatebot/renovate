...
const bitbucketApi = require('../platform/bitbucket');
...
async function parseConfigs(env, argv) {
  ...
  if (config.platform === 'github') {
    ...
    const { user, pswd } = config;
    if ((!config.token && !env.BB_TOKEN) || (!(user && pswd))) {
      throw new Error('You need to supply a BitBucket token, or BitBucker `--user` and `--pswd`');
    }
    const base64 = str => Buffer.from(str, 'binary').toString('base64');
    process.env.BB_TOKEN = config.token || env.BB_TOKEN || base64(`${user}:${pswd}`);
  } else {
  ...
  if (config.autodiscover) {
    ...
    if (config.platform === 'github') {
      ...
      logger.info('Autodiscovering BitBucket repositories');
      config.repositories = await bitbucketApi.getRepos(
        config.token,
        config.endpoint
      );
    }
