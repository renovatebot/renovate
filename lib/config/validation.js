...
function validateConfig(config, logger = config.logger) {
  let errors = [];
  let warnings = [];
  ...
  for (const key of Object.keys(config)) {
    ...
    if (
      ...
      if (!optionTypes[key]) {
        ...
        if (!hasValidSchedule(val, logger)) {
          errors.push({
            depName: 'Configuration Error',
            message: `Invalid schedule: \`${val}\``,
          });
        }
      } else if (val != null) {
        ...
        if (type === 'boolean') {
          ...
          if (!Array.isArray(val)) {
            ...
            val.forEach(subval => {
              if (isObject(subval)) {
                const subValidation = module.exports.validateConfig(
                  subval,
                  logger
                );
          ...
          if (isObject(val)) {
            const subValidation = module.exports.validateConfig(val, logger);
            warnings = warnings.concat(subValidation.warnings);
            errors = errors.concat(subValidation.errors);
          } else {
            errors.push({
