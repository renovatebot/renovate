...
function validateConfig(config) {
  ...
  for (const [key, val] of Object.entries(config)) {
    if (
      ...
      if (!optionTypes[key]) {
      ...
      } else if(key === 'packageRules' && val[0] !== undefined) {
        ...
        for(let prop in val[0]){
          if(selectorsToValidate.indexOf(prop) >= 0 && val[0][prop].length > 0){
            checkPackage = true;
            break;
          }
        }
        ...
        if(!checkPackage){
          errors.push({
            depName: 'Configuration Error',
            message: `The package doesn\'t contain any of required selectors`,
          });
        }
      } else if (val != null) {
