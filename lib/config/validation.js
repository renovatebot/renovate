    ...
    if (
      ...
      if (!optionTypes[key]) {
        ...
        if(Array.isArray(val)){
          const selectorsToValidate = ['packageNames', 'packagePatterns', 'excludePackageNames', 'excludePackagePatterns'];
          let checkPackage = false;

          for(let i = 0; i < val.length; i++){
            for(const prop in val[i]){
              if(selectorsToValidate.indexOf(prop) >= 0 && val[i][prop].length > 0){
                checkPackage = true;
                break;
              }
            }
          ...
          errors.push({
            depName: 'Configuration Error',
            message: `packageRule must be an array.`,
          });
        }
