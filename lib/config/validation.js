    ...
    if (
      ...
      if (!optionTypes[key]) {
        ...
        if (type === 'boolean') {
          ...
          if (!Array.isArray(val)) {
            ...
            for (const subval of val) {
          ...
          } else if(key === 'packageRules' && val.length > 0) {
              const selectorsToValidate = ['packageNames', 'packagePatterns', 'excludePackageNames', 'excludePackagePatterns', 'extends'];
              let checkPackage = false;

              val.forEach(obj => {
                if(isObject(obj)){
                  for(const subval in obj){
                    const subValidation = module.exports.validateConfig(obj);
                    warnings = warnings.concat(subValidation.warnings);
                    errors = errors.concat(subValidation.errors);
                    if(selectorsToValidate.indexOf(subval) >= 0 && obj[subval].length > 0){
                      checkPackage = true;
                    }
                  }
                }else{
                  errors.push({
                    depName: 'Configuration Error',
                    message: `packageRules must contain objects`,
                  });
                }
              });
              ...
              if(!checkPackage){
                errors.push({
                  depName: 'Configuration Error',
                  message: `Each packageRule must contain at least one selector`,
                });
              }
          } else {
