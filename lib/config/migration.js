...
function migrateConfig(config) {
  try {
    ...
    for (const [key, val] of Object.entries(config)) {
      if (removedOptions.includes(key)) {
      ...
      } else if (key === 'packageFiles' && Array.isArray(val)) {
        isMigrated = true;
        const fileList = [];
        ...
        isMigrated = true;
        migratedConfig.baseBranches = Array.isArray(val) ? val : [val];
        ...
        key === 'prBody' ||
        (key.startsWith('commitMessage') && val.includes('currentVersion'))
      ) {
        isMigrated = true;
        migratedConfig[key] = val.replace(/currentVersion/g, 'currentValue');
      }
    }
