...
function resolvePresets(inputConfig, inputLogger) {
  const logger = inputLogger || inputConfig.logger;
  ...
  for (const key of Object.keys(config)) {
    if (isObject(config[key])) {
      ...
      config[key] = resolvePresets(config[key], logger);
    }
  }
  return config;
}
