...
const options = [
  ...
  },
  {
    ...
    type: 'string',
    default: '{{#if isPin}}Pin{{else}}Update{{/if}} dependency {{depName}} to version {{#if isMajor}}{{newVersionMajor}}.x{{else}}{{newVersionRange}}{{/if}}',
    cli: false,
    env: false,
  },
  {
    ...
    type: 'string',
    default: 'This Pull Request updates dependency {{depName}} from version `{{currentVersion}}` to `{{newVersionRange}}`\n\n{{changelog}}',
    cli: false,
    env: false,
  },
