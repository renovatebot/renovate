...
function setBranchStatus(branchName, context, description, state, targetUrl) {
  logger.debug(
    `setBranchStatus(${branchName}, ${context}, ${description}, ${state}, ${targetUrl}) - Not supported by VSTS (yet! 01)`
  );
}

async function ensureCommentRemoval(issueNo, topic) {
  logger.info(
    `ensureCommentRemoval(issueNo, topic)(${issueNo}, ${topic}) - Not supported by VSTS (yet! 02)`
  );
  await null;
}
...
async function getAllRenovateBranches(branchPrefix) {
  logger.info(
    `getAllRenovateBranches(branchPrefix)(${branchPrefix}) - Not supported by VSTS (yet! 03)`
  );
  await null;
}
...
async function deleteBranch(branchName) {
  logger.info(
    `deleteBranch(branchName)(${branchName}) - Not supported by VSTS (yet! 05)`
  );
  await null;
}
...
async function mergeBranch(branchName, mergeType) {
  logger.info(
    `mergeBranch(branchName, mergeType)(${branchName}, ${mergeType}) - Not supported by VSTS (yet! 06)`
  );
  await null;
}
...
async function getBranchLastCommitTime(branchName) {
  logger.info(
    `getBranchLastCommitTime(branchName)(${branchName}) - Not supported by VSTS (yet! 07)`
  );
  await null;
}
...
async function addAssignees(issueNo, assignees) {
  logger.info(
    `addAssignees(issueNo, assignees)(${issueNo}, ${assignees}) - Not supported by VSTS (yet! 08)`
  );
  await null;
}
...
async function addReviewers(issueNo, reviewers) {
  logger.info(
    `addReviewers(issueNo, reviewers)(${issueNo}, ${reviewers}) - Not supported by VSTS (yet! 09)`
  );
  await null;
}

async function mergePr(pr) {
  logger.info(`mergePr(pr)(${pr}) - Not supported by VSTS (yet! 10)`);
  await null;
}
