...
async function commitFilesToBranch(
  ...
  let branchRef = await vstsHelper.getVSTSBranchObj(config.repoId, branchName, parentBranch);
  
  // create commits
  for (const file of files) {
    const isBranchExisting = await branchExists(`refs/heads/${branchName}`);
    if (isBranchExisting) {
      branchRef = await vstsHelper.getVSTSBranchObj(config.repoId, branchName, branchName);
    }
