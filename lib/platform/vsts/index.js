...
module.exports = {
  ...
  getBranchPr, // Done
  getBranchStatus, // Done
  getBranchStatusCheck, // Done
  ...
  // Comments
  ensureComment, // Done (but not working)
  ...
  // file
  getSubDirectories, // Not implemented yet
  commitFilesToBranch, // Done
  getFile, // should not be exported no? (now only getFileContent is used)
  getFileContent, // Done
  getFileJson, // Done
  // Commits
...
async function getFile(filePath, branchName = config.baseBranch) {
  ...
  const fc = await getFileContent(filePath, branchName);
  return fc;
}
...
async function ensureCommentRemoval(issueNo, topic) {
  logger.info(
    `ensureCommentRemoval(issueNo, topic)(${issueNo}, ${
      topic
...
async function getAllRenovateBranches(branchPrefix) {
  logger.info(
    `getAllRenovateBranches(branchPrefix)(${
      branchPrefix
    }) - Not supported by VSTS (yet!)`
  );
  await null;
...
async function getBranchLastCommitTime(branchName) {
  logger.info(
    `getBranchLastCommitTime(branchName)(${
      branchName
    }) - Not supported by VSTS (yet!)`
  );
  await null;
...
async function getSubDirectories(path) {
  logger.info(
    `getSubDirectories(path)(${path}) - Not supported by VSTS (yet!)`
  );
  await null;
}
