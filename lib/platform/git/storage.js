  ...
  constructor() {
    ...
    async function initRepo(args) {
      ...
      if (clone) {
        ...
        try {
          await git.clone(config.url, '.', ['--depth=2']);
        } catch (err) /* istanbul ignore next */ {
    ...
    async function setBranchPrefix(branchPrefix) {
      ...
      const cmd = `git fetch --depth=2 origin ${ref}`;
      // We do this exec because the simple-git library didn't seem to support passing through the escaped ref properly
      const { stdout, stderr } = await exec(cmd, {
        cwd: config.localDir,
        shell: true,
      });
      logger.debug({ stdout, stderr }, 'setBranchPrefix result');
    }
    ...
    async function branchExists(branchName) {
      ...
      if (!branchName.startsWith(config.branchPrefix)) {
        if (config.branchPrefix) {
          // TODO: remove this once we're happy that this only happens when setting base branches
          logger.info(
            { branchName, branchPrefix: config.branchPrefix },
            'Fetching branch not covered by branchPrefix'
          );
        }
        await git.fetch(config.url, ['--depth=2', 'origin', branchName]);
      }
      try {
        ...
        console.warn(await git.raw(['branch', '-a']));
