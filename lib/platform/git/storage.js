...
class Storage {
  constructor() {
    ...
    async function initRepo(args) {
      cleanRepo();
      config = { ...args };
      logger.info({ config }, 'git.fs.initRepo');
      cwd = config.localDir;
      ...
      if (clone) {
        ...
        try {
          logger.debug({ url: config.url }, 'this is the url were cloning');
...
Storage.getUrl = ({ gitFs, hostname, repository, auth, host }) => {
  ...
  return URL.format({
    protocol,
    auth,
    hostname, // doesnt work when host includes port
    host,
