...
const InternalGit = require('simple-git/src/git');
const Git = require('simple-git/promise');
const convertHrtime = require('convert-hrtime');
const { Container } = require('./docker');

InternalGit.prototype.customSpawn = function customSpawn(spawn) {
...
class Storage {
  constructor() {
    ...
    async function initRepo(args) {
      ...
      if (process.env.RENOVATE_GIT_FS_DOCKER) {
        container = new Container();
        await container.create(cwd);
        git.customSpawn(container.spawn.bind(container));
      }
      // istanbul ignore if
