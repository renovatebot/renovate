...
async function getFileList(branchName) {
  ...
  try {
    ...
    config.fileList = fileListRaw.map(file => file.path);
  } catch (err) /* istanbul ignore next */ {
    logger.info(
...
function mergeBranch() {
  ...
  throw new Error('Branch automerge not supported');
}
...
function ensureIssue() {
  ...
  loggger.warn('Cannot ensure issues');
}
...
function addReviewers() {
  ...
  loggger.warn('Cannot add reviewers');
}
...
async function commitFilesToBranch(
  ...
  const branchHead = await getBranchCommit(branchName);
  const parentHead = await getBranchCommit(parentBranch);
  const parents = branchHead || parentHead;
