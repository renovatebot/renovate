...
async function getBranchStatus(branchName, requiredStatusChecks) {
  ...
  if (!isCloud) {
    // there is a not documented endpoint, which doesn seeem to work
    // const yolo = api.get('/rest/build-status/latest/commits/stats', { json: [sha] });
    logger.info(
      'Bitbucket server doesnt publicly expose status check = returning "failed"'
    );
    // do not harm
    return 'failed';
  }
