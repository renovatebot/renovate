...
// TODO: add recursive util for bb-got
...
async function initRepo(repoName, token, endpoint) {
  ...
  try {
    const info = utils.repoInfoTransformer(
      (await api.get(`/repositories/${repoName}`)).body
    );
...
async function getFileList(branchName = config.baseBranch) {
  ...
  try {
    const fileListRaw = (await api.get(
      `/2.0/repositories/${config.repoName}/src/${branchName}/`
    )).body.values;
    const fileList = fileListRaw
      .filter(file => !R.propEq('type', 'commit_directory', file))
      .map(R.prop('path'));
...
async function getAllRenovateBranches(branchPrefix) {
  ...
  const allBranches = (await api.get(`/2.0/repositories/${config.repoName}/refs/branches`)).body.values || [];
...
async function isBranchStale(branchName) {
  ...
  const branches =
    (await api.get(`/2.0/repositories/${config.repoName}/refs/branches`)).body
      .values || [];
  const branch = branches.find(R.propEq('name', branchName)) || { target: {} };

  const commits = (await api.get(utils.addMaxLength(branch.links.commits.href)))
    .body.values;
  const repoInfo =
    (await api.get(`/2.0/repositories/${config.repoName}`)).body || {};
  const mainbranch = (repoInfo.mainbranch || {}).name || 'master';
  const mainbranchCommits = (await api.get(
    utils.addMaxLength(
...
async function getBranchStatus(branchName, requiredStatusChecks) {
  ...
  const branches =
    (await api.get(
      utils.addMaxLength(`/2.0/repositories/${config.repoName}/refs/branches`)
    )).body.values || [];
  const branch = branches.find(R.propEq('name', branchName));
  const statuses =
    (await api.get(branch.target.links.statuses.href)).body.values || [];
...
async function getBranchStatusCheck(branchName) {
  ...
  const statuses = (await api.get(branch.target.links.statuses.href)).body
    .values;
  return statuses.map(R.prop('state')).every(x => x === 'SUCCESSFUL');
}
...
async function getBranchLastCommitTime(branchName) {
  try {
    const branches =
      (await api.get(`/repositories/${config.repoName}/refs/branches`)).body
        .values || [];
    const branch = branches.find(R.propEq('name', branchName)) || {
      target: {},
    };
...
async function createPr(
  ...
  const prInfo = (await api.post(
    `/repositories/${config.repoName}/pullrequests`,
    { body }
  )).body;
  return prInfo.id;
}
...
async function getPr(prNo) {
  ...
  const commits = (await api.get(utils.addMaxLength(pr.links.commits.href)))
    .body.values;
...
async function getCommitMessages() {
  ...
  try {
    const { values } = (await api.get(utils.addMaxLength(`/2.0/repositories/${config.repoName}/commits`))).body;
