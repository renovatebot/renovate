...
async function getFileList(branchName = config.baseBranch) {
  ...
  try {
    const fileListRaw = await utils.files(
      `/2.0/repositories/${config.repository}/src/${branchName}/`
    );
...
async function isBranchStale(branchName) {
  ...
  const [branch, baseBranch] = (await Promise.all([
    ...
    api.get(
      `/2.0/repositories/${config.repository}/refs/branches/${config.baseBranch}`
    )
  ])).map(R.prop('body'))
...
async function createPr(
  ...
  const repoInfo = (await api.get(`/2.0/repositories/${config.repository}`)).body;
...
async function commitFilesToBranch(
  ...
  const form = utils.commitForm({ message, gitAuthor, parents, branchName, files });
...
async function getFile(filePath, branchName) {
  ...
  try {
    const file = (await api.get(
      `/2.0/repositories/${config.repository}/src/${branchName || config.baseBranch}/${filePath}`,
      { json: false }
    )).body;
    return file;
  } catch (err) {
...
async function getCommitMessages() {
  ...
  try {
    const values = await utils.accumulateValues(`/2.0/repositories/${config.repository}/commits`);
