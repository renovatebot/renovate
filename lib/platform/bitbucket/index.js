...
async function isBranchStale(branchName) {
  ...
  if (isCloud) {
    ...
    // TODO: Disable lint suppression below when refactoring and covering for both cloud and server implementations
...
async function deleteBranch(branchName, closePr) {
  const isCloud = false; // TODO: fixme
  if (isCloud) {
  ...
  if (closePr) {
    ...
    await api.post(
      `/rest/api/1.0/projects/${projectKey}/repos/${repositorySlug}/pull-requests/${
        pr.number
      }/decline?version=${pr.version + 1}` // TODO: fix off-by-one
    );
  }
  return api.delete(
    // TODO: use gitFS
