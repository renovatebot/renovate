  ...
  try {
    ...
    if (opts.paginate) {
      ...
      if (linkHeader && linkHeader.next && linkHeader.last) {
        ...
        try {
          const cacheVal = await cacache.get(paginationCache, path);
          const cacheResult = JSON.parse(cacheVal.data.toString());
          if (cacheResult.etag === etag) {
            logger.debug({ path }, 'Returning cached paginated result');
            res.body = cacheResult.body;
            return res;
          }
          logger.debug({ path }, 'Outdated pagination cache');
        } catch (err) {
          logger.debug({ path }, 'Paginated cache miss');
        }
        ...
        logger.debug({ path }, 'Caching paginated result');
        await cacache.put(
          paginationCache,
          path,
          JSON.stringify({ etag, body: res.body })
        );
      }
    }
    if (
