...
const { defaults } = require('../../util/host-rules');

const APIurlGitHub = defaults.github.endpoint;
let cache = {};
let stats = {};
...
async function get(path, options, retries = 5) {
  ...
  try {
    ...
    const gotPath = path.includes(APIurlGitHub) ? path : APIurlGitHub + path;
    const res = await got(gotPath, { ...gotOpts, ...opts });
    if (res && res.headers) {
