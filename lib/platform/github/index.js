...
async function createBranch(branchName, sha) {
  ...
  try {
    ...
    if (err.statusCode === 422) {
      logger.warn({ err, message: err.message }, 'Error creating branch');
      try {
        await updateBranch(branchName, sha);
        logger.info('Updated branch after failed create');
        return;
      } catch (err2) {
        logger.warn(
          { err: err2, message: err2.message },
          'Error updating branch after failed branch creation'
        );
      }
      throw new Error('repository-changed');
    }
    logger.info('Unknown error creating branch');
    throw err;
  }
}
