...
async function initRepo({ repository, token, endpoint, forkMode, forkToken }) {
  ...
  try {
    ...
    if (err.message === 'Repository access blocked') {
      throw new Error('blocked');
    }
    logger.info({ err, body: res.body }, 'Unknown GitHub initRepo error');
    throw err;
  }
...
async function branchExists(branchName) {
  ...
  config.branchList =
    config.branchList ||
    (await get(`repos/${config.repository}/branches?per_page=100`, {
      paginate: true,
    })).body.map(branch => branch.name);
  const res = config.branchList.includes(branchName);
  logger.debug(`branchExists=${res}`);
  return res;
}
