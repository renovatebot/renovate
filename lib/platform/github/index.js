  ...
  if (!config.openPrList) {
    ...
    try {
      ...
      for (const pr of res.data.repository.pullRequests.nodes) {
        ...
        pr.canMerge =
          canMergeStates.includes(pr.mergeStateStatus) &&
...
function hasNegativeReview(reviews) {
  const negativeReivews = {};
  for (const review of reviews || []) {
    negativeReivews[review.author.login] = review.state;
    if (review.state === 'CHANGES_REQUESTED') {
      negativeReivews[review.author.login] = review.state;
    } else if (review.state === 'APPROVED') {
      if (negativeReivews[review.author.login]) {
        delete negativeReivews[review.author.login];
      }
    }
  }
  return Object.keys(negativeReivews).length > 0;
}
