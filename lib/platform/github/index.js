...
const { graphql } = require('./gh-graphql-wrapper');
...
async function getIssueList() {
  ...
  if (!config.issueList) {
    ...
    graphql.setRepoOwner(config.repositoryOwner);
    graphql.setRepoName(config.repositoryName);
    graphql.setRepoItem('issues');

    graphql.setRepoItemFilterBy(`{createdBy: "${config.renovateUsername}"}`);

    graphql.setRepoItemNodeList(['number', 'state', 'title', 'body']);

    graphql.setRepoResultNumEls(100);

    config.issueList = await graphql.getAll();
