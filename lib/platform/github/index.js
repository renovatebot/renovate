...
async function closeMissingDeprecatedIssue(deprecatedPackages, manager) {
  logger.debug(`closeMissingDeprecatedIssue()`);
  const issueList = await getIssueList();
  const titleMatch = 'Dependency deprecation warning';
  const issues = issueList.filter(i => i.title.includes(titleMatch));
  for (const i of issues) {
    if (
      i.state === 'open' &&
      !isDeprecatePackageExist(i.title, deprecatedPackages, manager)
    ) {
      await closeIssue(i.number);
    }
  }
...
function isDeprecatePackageExist(title, deprecatedPackages, manager) {
  for (const [depName] of Object.entries(deprecatedPackages)) {
