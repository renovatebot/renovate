...
async function createPr(branchName, title, body, labels, useDefaultBranch) {
  ...
  // If we are in fork mode, the repository to create PR in will be the parent repository, not current repository
  const repository = config.parentRepo ? config.parentRepo : config.repoName;
  // Include the repository owner to handle forkMode and regular mode
  const head = `${config.repoName.split('/')[0]}:${branchName}`;
  const pr = (await get.post(`repos/${repository}/pulls`, {
    body: {
      title,
      head,
      base,
