...
async function getAuthor(token, endpoint) {
  logger.info('Getting github author from api');
  const opts = hostRules.find({ platform: 'github' }, { token, endpoint });
  if (!opts.token) {
    throw new Error('No token found for getAuthor');
  }
  hostRules.update({ ...opts, platform: 'github', default: true });
  try {
    const res = await get('user', { paginate: true });
    return { name: res.body.name, email: res.body.email };
  } catch (err) /* istanbul ignore next */ {
    logger.error({ err }, `GitHub getAuthor error`);
    return null;
  }
}
function cleanRepo() {
