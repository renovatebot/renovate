...
async function getIssueList() {
  if (!config.issueList) {
    const res = await get(
      `projects/${config.parentRepo || config.repository}/issues?state=opened`
    );
    // istanbul ignore if
  ...
  try {
    ...
    if (issue) {
      const issueBody = (await get(
        `projects/${config.parentRepo || config.repository}/issues/${issue.iid}`
      )).body.body;
      if (issueBody !== body) {
        logger.debug('Updating issue body');
        await get.put(
          `projects/${config.parentRepo || config.repository}/issues/${
            issue.iid
          }`,
          {
