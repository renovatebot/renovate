...
async function getComments(issueNo) {
  ...
  const url = `https://gitlab.com/api/v4/projects/${config.parentRepo ||
    config.repository}/merge_requests/${issueNo}/notes`;
...
async function addComment(issueNo, body) {
  ...
  await get.post(
    `https://gitlab.com/api/v4/projects/${config.parentRepo ||
      config.repository}/merge_requests/${issueNo}/notes`,
    {
      body: { body },
    }
...
async function editComment(issueNo, commentId, body) {
  ...
  await get.patch(
    `https://gitlab.com/api/v4/projects/${config.parentRepo ||
      config.repository}/merge_requests/${issueNo}/notes/${commentId}`,
    {
      body: { body },
    }
  );
