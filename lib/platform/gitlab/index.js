...
async function checkCompatibility(branchName, targetBranch){
  logger.info(`Checking branch compatibility between source: ${branchName} , target: ${targetBranch}`);
  try{
    const targetLastCommit = await getBranchCommit(targetBranch, 'last');
    console.log(targetLastCommit);
    const sourceCommits = await getBranchCommit(branchName, 'all');
    console.log(sourceCommits);
    if(sourceCommits.includes(targetLastCommit)){
      return true;
    }
  }catch(err){
    logger.info(
...
async function mergeBranch(branchName, mergeType) {
  ...
  if (mergeType === 'branch') {
    ...
    if(checkCompatibility(branchName, config.baseBranch)){
      try {
        await get.post(
          `projects/${
            config.repository
