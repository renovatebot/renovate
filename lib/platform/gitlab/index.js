...
async function checkCompatibility(branchName, targetBranch) {
  ...
  try {
    const targetLastCommit = (await getBranchCommit(targetBranch))[0];
    const sourceCommit = (await getBranchCommit(branchName))[1];
    return sourceCommit.id === targetLastCommit.id;
  } catch (err) {
    logger.info(
      expandError(err),
...
async function mergeBranch(branchName) {
  ...
  console.log('branchURI: ' + branchURI);
