...
async function checkCompatibility(branchName, targetBranch) {
  logger.info(
    `Checking branch compatibility between source: ${branchName} , target: ${targetBranch}`
  );
  let isCompatible = false;
  try {
    const targetLastCommit = await getBranchCommit(targetBranch, 'last');
    const sourceCommits = await getBranchCommit(branchName, 'all');
    sourceCommits.forEach(el => {
      if (el.id === targetLastCommit.id) {
        isCompatible = true;
      }
    });
  } catch (err) {
    logger.info(
      expandError(err),
      `Error checking compatibility (${branchName}, ${targetBranch})`
    );
    throw new Error('Branch merge compatibility check failed');
  }
  return isCompatible;
}
...
async function mergeBranch(branchName) {
  ...
  const isCompatible = await checkCompatibility(branchName, config.baseBranch);
  if (!isCompatible) {
    logger.info('Branch is not compatible with merge - needs rebasing');
    return;
  }
  try {
    await get.post(
...
async function getBranchCommit(branchName, commitPosition = 'last') {
  try {
    const res = await get(
      `projects/${config.repository}/repository/commits?ref_name=${branchName}`
    );
    if (commitPosition === 'last') {
      return res.body[0];
    } else if (commitPosition === 'all') {
      return res.body;
    }
    return null;
