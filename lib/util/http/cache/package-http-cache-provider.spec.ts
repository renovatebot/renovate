...
describe('util/http/cache/package-http-cache-provider', () => {
  ...
  beforeEach(() => {
    ...
    packageCache.setRawTtl.mockImplementation((_ns, k, v, _ttl) => {
  ...
  it('loads cache bypassing server', async () => {
    ...
    expect(packageCache.setRawTtl).not.toHaveBeenCalled();
    ...
    expect(packageCache.setRawTtl).toHaveBeenCalled();
  });
