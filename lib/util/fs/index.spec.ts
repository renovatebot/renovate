...
describe('util/fs/index', () => {
  ...
  describe('readLocalFile', () => {
    ...
    it('logs a warning if hidden Unciode characters are found', async () => {
      ...
      // NOTE that we cannot test that this is called once across multiple
      // `readLocalFile` as we're mocking the logger, so we're testing that it's
      // called, not validating the nubmer of times
    ...
    it('logs a trace message (not warning) if hidden Unicode characters are found in a binary file', async () => {
      // Create a binary file with null bytes (making it binary) and zero-width
      // space (U+200B)
