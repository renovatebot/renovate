const ghGot = require('../../platform/github/gh-got-wrapper').api;
...
async function getTagCommit(githubRepo, tag) {
  ...
  try {
    ...
    const res = (await ghGot.get(url)).body.object;
    if (res.type === 'commit') {
      digest = res.sha;
    } else if (res.type === 'tag') {
      digest = (await ghGot.get(res.url)).body.object.sha;
    } else {
...
async function getDigest({ lookupName: githubRepo }, newValue) {
  ...
  try {
    ...
    digest = (await ghGot.get(url)).body[0].sha;
  } catch (err) {
    logger.info(
      { githubRepo, err },
...
async function getPkgReleases({ lookupName: repo, lookupType }) {
  ...
  try {
    if (lookupType === 'releases') {
      ...
      versions = (await ghGot.get(url, { paginate: true })).body.map(
        o => o.tag_name
      );
    } else {
      // tag
      const url = `https://api.github.com/repos/${repo}/tags?per_page=100`;
      versions = (await ghGot.get(url, {
        paginate: true,
      })).body.map(o => o.name);
    }
  } catch (err) {
