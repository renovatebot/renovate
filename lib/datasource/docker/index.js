...
async function getAuthHeaders(registry, repository) {
  ...
  try {
    ...
    const hostOpts = hostRules.find({ hostType, url: apiCheckUrl });
    hostOpts.json = true;
    if (hostOpts.username && hostOpts.password) {
      const auth = Buffer.from(
        `${hostOpts.username}:${hostOpts.password}`
      ).toString('base64');
      hostOpts.headers = { Authorization: `Basic ${auth}` };
    }

    if (authenticateHeader.scheme.toUpperCase() === 'BASIC') {
      logger.debug(`Using Basic auth for docker registry ${repository}`);
      await got(apiCheckUrl, hostOpts);
      return hostOpts.headers;
    }
    ...
    const { token } = (await got(authUrl, hostOpts)).body;
    // istanbul ignore if
    if (!token) {
