...
function parseImage(image, registryUrls) {
  const res = {};
  if (!image) {
    return res;
  }
  ...
  if (split.length > 1 && split[0].includes('.')) {
    res.registry = split.shift();
  }
  if (!res.registry || res.registry === 'docker.io') {
    if (is.nonEmptyArray(registryUrls)) {
      res.registry = registryUrls[0];
    } else {
      res.registry = 'index.docker.io';
    }
    if (split.length === 1) {
      split.unshift('library');
    }
  }
  if (!res.registry.match('$https?://')) {
    res.registry = `https://${res.registry}`;
  }
  res.repository = split.join('/');
  return res;
}
...
async function getDigest({ lookupName, registryUrls }, newValue) {
  const { registry, repository } = parseImage(lookupName, registryUrls);
