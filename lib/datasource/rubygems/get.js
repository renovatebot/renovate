...
const { wrap, httpCache } = require('./cacheable');
...
const isCompatible = ({ ruby }, { rubyVersion }) =>
  !ruby ? true : satisfies(ruby, rubyVersion);
...
const fetch = async ({ dependency, registry, path }) => {
  ...
  const cache = process.env.RENOVATE_SKIP_CACHE ? undefined : httpCache;

  const name = `/${dependency}.json`;
  const baseUrl = `${registry}/${path}`;

  const response =
    (await got(name, { cache, retry, json, baseUrl, headers })) || {};

  return response.body;
};

const getDependency = async ({ dependency, registry, compatibility }) => {
  try {
    ...
    const releases = versions
      ...
      .filter(isCompatible.bind(null, compatibility));
...
module.exports = {
  getDependency: wrap()(getDependency),
};
