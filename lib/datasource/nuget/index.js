...
async function getPkgReleasesFromV3Feed(registryUrl, feedUrl, pkgName) {
  ...
  try {
    const pkgUrlListRaw = await authenticatedGot(queryUrl, {
      retry: 5,
      json: true,
    });
    ...
    try {
      ...
      if (registryUrl.toLowerCase() === defaultNugetFeed.toLowerCase()) {
        ...
        const result = await authenticatedGot(nugetOrgApi);
...
async function getPkgReleasesFromV2Feed(feedUrl, pkgName) {
  ...
  try {
    const pkgVersionsListRaw = await authenticatedGot(pkgUrlList, { retry: 5 });
    const pkgLatestRaw = await authenticatedGot(pkgUrl, { retry: 5 });
...
async function getQueryUrlForV3Feed(url) {
  ...
  try {
    const servicesIndexRaw = await authenticatedGot(url, {
      retry: 5,
      json: true,
    });
...
function authenticatedGot(url, options) {
