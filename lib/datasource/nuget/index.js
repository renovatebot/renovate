  ...
  for (const feed of feeds) {
    ...
    if (feedVersion !== null) {
      if (feedVersion === 3) {
        ...
        if (queryUrl !== null) {
          dep = await getPkgReleasesFromV3Feed(queryUrl, pkgName);
        }
      } else if (feedVersion === 2) {
        dep = await getPkgReleasesFromV2Feed(feed, pkgName);
      }
    }
  }
  if (dep != null) {
  ...
  try {
    ...
    try {
      if (
        ...
        dep.sourceUrl = pkgUrlListRaw.body.data[0].projectUrl;
      } else {
        dep.sourceUrl = '';
      }
...
async function getPkgReleasesFromV2Feed(feedUrl, pkgName) {
  ...
  try {
    ...
    try {
      ...
      dep.sourceUrl = pkgInfo.children
