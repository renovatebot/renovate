...
async function getPkgReleases(purl, config) {
  ...
  for (let i = 0; i < repositories.length; i += 1) {
    ...
    if (mavenMetadata) {
      ...
      logger.debug(
        `Found ${newVersions.length} new versions for ${
          dependency.display
  ...
  if (versions.length === 0) {
    logger.warn(
      `No versions found for ${dependency.display} in ${
        repositories.length
      } repositories`
    );
    return null;
...
function getRepositories(purl) {
  return purl.qualifiers.repository_url.split(',');
}
...
function extractVersions(mavenMetadata) {
  ...
  return doc.metadata.versioning.versions.version.map(v => String(v));
}
