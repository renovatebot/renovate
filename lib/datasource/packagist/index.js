...
const hostType = 'packagist';

function getHostOpts(url) {
  const hostOpts = hostRules.find({ hostType, url });
  hostOpts.json = true;
  if (hostOpts && hostOpts.username && hostOpts.password) {
    const auth = Buffer.from(
      `${hostOpts.username}:${hostOpts.password}`
    ).toString('base64');
    hostOpts.headers = { Authorization: `Basic ${auth}` };
    delete hostOpts.username;
    delete hostOpts.password;
  }
  return hostOpts;
}
