...
function getClient(registry, repository) {

  const { host } = URL.parse(registry);
  const opts = hostRules.find({ platform: 'docker', host }, { json: true });

  const client = dockerRegistryClient.createClientV2({
...
async function getDigest(config, newValue) {
  ...
  const client = getClient(massagedRegistry, repository);
  client.getManifest[promisify.custom] = (opts) => {
    return new Promise((resolve, reject) => {
      client.getManifest(opts, (err, manifest, res, manifestStr) => {
        if (err) {
          reject(err);
        } else {
          resolve({manifest, res, manifestStr})
        }
      });
    });
  };
  client.getManifestAsync = promisify(client.getManifest);
  ...
  try {
    ...
    const { res: manifestResponse } = await client.getManifestAsync({ ref: newTag });
...
async function getTags(registry, repository) {
  ...
  const client = getClient(registry, repository);
  client.listTagsAsync = promisify(client.listTags);
  ...
  try {
    ...
    const { tags } = await client.listTagsAsync();
