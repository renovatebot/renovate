...
async function getPkgReleases(config) {
  ...
  repoRoots.forEach(repoRoot => {
    ...
    if (depType === 'plugin') {
      searchRoots.push(`${repoRoot}/${groupIdSplit.join('.')}/${artifact}`);
      searchRoots.push(`${repoRoot}/${groupIdSplit.join('/')}/${artifact}`);
    } else {
  ...
  for (let idx = 0; idx < searchRoots.length; idx += 1) {
    ...
    const versions =
      depType === 'plugin'
        ? await resolvePluginReleases(searchRoot, scalaVersion)
...
async function resolvePluginReleases(searchRoot, scalaVersion) {
