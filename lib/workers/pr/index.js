...
function getPrBody(config) {
  let prBody = '';
  // istanbul ignore if
  if (config.prBanner && !config.isGroup) {
    prBody += handlebars.compile(config.prBanner)(config) + '\n\n';
  }
  const hasDepTypes = config.upgrades.some(upgrade => upgrade.depType);
  const hasReferences = config.upgrades.some(
    upgrade => upgrade.homepage || upgrade.repositoryUrl || upgrade.changelogUrl
  );
  prBody += `| Package |${hasDepTypes ? ' Type |' : ''} Update | New value |${
  ...
  for (const upgrade of config.upgrades) {
    ...
    if (repositoryUrl) {
      ...
      references += `[source](${repositoryUrl})`;
    }
    if (changelogUrl) {
      if (references.length) {
        references += ', ';
      }
      references += `[changelog](${changelogUrl})`;
    }
    let value = '';
    if (newDigestShort) {
      if (config.isPin) {
    ...
    const name =
      upgrade.updateType === 'lockFileMaintenance'
        ...
        : '`' + depName + '`';
    // prettier-ignore
    prBody += `| ${name} | ${hasDepTypes ? depType + ' |' : ''} ${updateType} | ${value} |`;
    if (hasReferences) {
      prBody += references + ' |';
    }
    prBody += '\n';
