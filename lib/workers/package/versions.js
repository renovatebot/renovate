...
function determineUpgrades(npmDep, config) {
  ...
  if (!isValidVersion(currentVersion)) {
    ...
    if (knownTags.indexOf(currentVersion) === -1) {
      result.message = `Unsupported current version "${currentVersion}"`;
    } else {
      result.message = `Cannot renovate tag "${currentVersion}"`;
    }
    logger.warn(result.mesage);
    return [result];
  }
  const versions = npmDep.versions;
  if (!versions || Object.keys(versions).length === 0) {
    result.message = `No versions returned from registry`;
  ...
  if (semverParsed.length > 1) {
    ...
    result.message = `Unsupported semver range: "${currentVersion}"`;
    logger.warn(result.message);
    return [result];
  }
