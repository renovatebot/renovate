...
async function renovatePackage(config) {
  ...
  return results.map(result => {
    ...
    if (upg.isMajor) {
      upg = configParser.mergeChildConfig(config, config.major);
    } else if (upg.isMinor) {
      upg = configParser.mergeChildConfig(config, config.minor);
    } else if (upg.isPatch) {
      upg = configParser.mergeChildConfig(config, config.patch);
    }
    upg.repositoryUrl =
