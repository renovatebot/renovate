...
async function ensureBranch(upgrades) {
  ...
  // Check if this branch should be automerged
  const headUpgrade = Object.assign({}, upgrades[0]);
  // Use the first upgrade for all the templates
  const branchName = handlebars.compile(headUpgrade.branchName)(headUpgrade);
  // parentBranch is the branch we will base off
  // If undefined, this will mean the defaultBranch
  const parentBranch = await module.exports.getParentBranch(branchName, headUpgrade);
  const commitMessage = handlebars.compile(headUpgrade.commitMessage)(headUpgrade);
  const api = headUpgrade.api;
  const packageFiles = {};
  const commitFiles = [];
