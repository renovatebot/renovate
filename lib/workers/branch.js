...
async function getParentBranch(branchName, config) {
  ...
  if (
    ...
    logger.debug(
      `Branch ${branchName} is stale and needs rebasing before merging`
    );
    return undefined;
  }
  if (config.rebaseStalePrs && isBranchStale) {
...
async function ensureBranch(upgrades) {
  ...
  if (!config.automerge) {
    // No automerging, so return true to indicate branch exists
    return true;
  }
  if (
    config.automergeType !== 'branch-commit' &&
    config.automergeType !== 'branch-push'
  ) {
    // Automerging, but not branch type
    return true;
  }
  ...
  // Always return false as we don't want a PR
  return false;
}
