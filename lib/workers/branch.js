...
function useBaseBranch(pr, config) {
  // Decide if we need to rebase
  if (!pr) {
    logger.debug('useBaseBranch: No PR found');
    return false;
  }
  if (pr.isUnmergeable || (pr.isStale && config.rebaseStalePRs)) {
    ...
    if (pr.canRebase) {
      ...
      logger.debug('useBaseBranch: rebasing');
      return true;
    }
    // Don't do anything different, but warn
    logger.verbose('Cannot rebase branch');
  } else {
    logger.debug('Existing branch does not need rebasing');
  }
  return false;
}
...
async function ensureBranch(config) {
  ...
  if (await config.api.branchExists(branchName)) {
    ...
    if (useBaseBranch(pr, config) === false) {
