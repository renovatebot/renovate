...
async function ensureBranch(config) {
  ...
  if (branchExists) {
    await updateExistingBranch(config, branchName, config.depType, config.depName,
      config.newVersion, commitMessage);
    return;
  }
  await createNewBranch(config, branchName, config.depType, config.depName,
    config.newVersion, commitMessage);
}
