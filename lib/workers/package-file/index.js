...
function renovatePackageFile(config) {
  const { manager } = config;
  if (manager === 'bazel') {
    return renovateBazelFile(config);
  } else if (manager === 'docker') {
    return renovateDockerfile(config);
  } else if (manager === 'meteor') {
    return renovateMeteorPackageFile(config);
  } else if (manager === 'node') {
    return renovateNodeFile(config);
  } else if (manager === 'npm') {
...
async function renovatePackageJson(packageFileConfig) {
