...
function determineLockFileDirs(config) {
  ...
  if (config.updatedPackageFiles.length && config.lernaPackageLock) {
    lernaDirs.push('.');
  }
...
async function getUpdatedLockFiles(config) {
  ...
  if (dirs.lernaDirs.length) {
    logger.debug('Generating package-lock.json files using lerna');
    const res = await lerna.generateLockFiles(config.tmpDir.path, env);
    if (res.error) {
      lockFileErrors.push({
      ...
      for (const packageFile of config.packageFiles) {
        ...
        const filename = upath.join(baseDir, 'package-lock.json');
        ...
        if (existingContent) {
          ...
          const newPackageLockPath = upath.join(config.tmpDir.path, filename);
          logger.debug('Checking against ' + newPackageLockPath);
          const newContent = await fs.readFile(newPackageLockPath, 'utf8');
