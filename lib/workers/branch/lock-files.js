...
function determineLockFileDirs(config) {
  ...
  function getPackageFile(fileName) {
    ...
    for (const packageFile of config.upgrades[0].packageFiles) {
  ...
  for (const p of config.updatedPackageFiles) {
    ...
    // lerna first
    if (packageFile.lernaLockFile) {
      logger.debug(`${packageFile.packageFile} has lerna lock file`);
      if (!lernaDirs.includes(packageFile.lernaDir)) {
        lernaDirs.push(packageFile.lernaDir);
      }
    } else if (packageFile.workspaceDir) {
      logger.debug(`${packageFile.packageFile} has is part of workspaces`);
      if (!yarnLockFileDirs.includes(packageFile.workspaceDir)) {
        yarnLockFileDirs.push(packageFile.workspaceDir);
      }
    } else {
      if (packageFile.yarnLock) {
        logger.debug(`${packageFile.packageFile} has yarn lock file`);
        if (!yarnLockFileDirs.includes(path.dirname(packageFile.packageFile))) {
          yarnLockFileDirs.push(path.dirname(packageFile.packageFile));
        }
      }
      if (packageFile.packageLock) {
        logger.debug(`${packageFile.packageFile} has package lock file`);
        if (
          !packageLockFileDirs.includes(path.dirname(packageFile.packageFile))
        ) {
          packageLockFileDirs.push(path.dirname(packageFile.packageFile));
        }
      }
      if (packageFile.shrinkwrapJson) {
        logger.debug(`${packageFile.packageFile} has npm shrinkwrap file`);
        if (
          !npmShrinkwrapDirs.includes(path.dirname(packageFile.packageFile))
        ) {
          npmShrinkwrapDirs.push(path.dirname(packageFile.packageFile));
        }
      }
      if (packageFile.shrinkwrapYaml) {
        logger.debug(`${packageFile.packageFile} has pnpm shrinkwrap file`);
        if (
          !shrinkwrapYamlDirs.includes(path.dirname(packageFile.packageFile))
        ) {
          shrinkwrapYamlDirs.push(path.dirname(packageFile.packageFile));
        }
      }
    }
  }
