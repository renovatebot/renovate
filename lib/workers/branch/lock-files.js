      ...
      if (toCopy.length !== 0) {
        ...
        await Promise.all(
          toCopy.map(async localPath => {
            ...
            // at the root of local Lib we should find a package.json and yarn will use it to update yarn.lock file
            const resolvedRepoPath = upath.join(resolvedLocalPath.substring(config.tmpDir.path.length + 1), 'package.json');
            ...
            if (fileContent !== null) {
              ...
              logger.error(`listOfNeededLocalFiles - file '${resolvedRepoPath}' not found.`)
            }
          })
        );
      }
