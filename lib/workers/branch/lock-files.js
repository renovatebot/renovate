...
function determineLockFileDirs(config) {
  ...
  for (const packageFile of Object.keys(config.updatedPackageFiles)) {
    if (module.exports.hasYarnLock(config, packageFile)) {
      yarnLockFileDirs.push(path.dirname(packageFile));
    }
    if (module.exports.hasPackageLock(config, packageFile)) {
      packageLockFileDirs.push(path.dirname(packageFile));
    }
  }
...
async function getUpdatedFiles(config) {
  const {
    packageLockFileDirs,
    yarnLockFileDirs,
  ...
  for (const lockFileDir of packageLockFileDirs) {
    try {
      const newContent = await npm.generateLockFileNew(
  ...
  for (const lockFileDir of yarnLockFileDirs) {
    try {
      const newContent = await yarn.generateLockFileNew(
        path.join(config.tmpDir.name, lockFileDir)
      );
