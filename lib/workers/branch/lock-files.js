...
async function writeExistingFiles(config) {
  ...
  for (const packageFile of config.packageFiles) {
    ...
    if (packageFile.packageFile.endsWith('package.json')) {
      ...
      // if you have some reference to local files, copy them
      const toCopy = listLocalTgz(massagedFile.dependencies);
      toCopy.push(...listLocalTgz(massagedFile.devDependencies));
      if (toCopy.length !== 0) {
        await Promise.all(
          toCopy.map(async localPath => {
            const resolvedLocalPath = path.resolve(basedir, localPath);
            // upath.join here to ensure / in path
            ...
            await fs.outputFile(resolvedLocalPath, fileContent);
          })
        );
      }
    }
...
function listLocalTgz(dependencies) {
  logger.trace(`listLocalTgz (${dependencies})`);
  const toCopy = [];
  for (const libName in dependencies) {
    if ({}.hasOwnProperty.call(dependencies, libName)) {
      const libVersion = dependencies[libName];
      if (libVersion.startsWith('file:')) {
        if (libVersion.endsWith('.tgz')) {
          toCopy.push(libVersion.substring(5));
        } else {
          logger.error(
            `local folder (${
            libVersion
            }) is not supported. Please do it like: 'file:/path/to/local/tarball.tgz'`
          );
        }
      }
    }
  }
  logger.debug(`listOfNeededLocalFiles toCopy: ${toCopy}`);
  return toCopy;
}
