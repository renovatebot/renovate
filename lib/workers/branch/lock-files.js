...
function determineLockFileDirs(config, deps) {
  const npmLockDirs = [];
  const yarnLockDirs = [];
  ...
  for (const upgrade of config.upgrades) {
    if (upgrade.type === 'lockFileMaintenance') {
      ...
      for (const packageFile of deps.npm) {
...
async function writeExistingFiles(config, deps) {
...
async function getUpdatedLockFiles(config, deps) {
  ...
  const dirs = module.exports.determineLockFileDirs(config, deps);
  logger.debug({ dirs }, 'lock file dirs');
  await module.exports.writeExistingFiles(config, deps);
  await module.exports.writeUpdatedPackageFiles(config, deps);
