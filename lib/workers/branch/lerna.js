...
async function generateLockFiles(manager, tmpDir, env) {
  ...
  try {
    ...
    try {
      ...
      npxCmd = `node ${installedPath}`.replace('npm-cli', 'npx-cli');
    } catch (err) {
      logger.warn('Could not find globally installed npm');
      npxCmd = 'npx';
    }
    logger.debug(`Using npx: ${npxCmd}`);
    ...
    const cmd = `${npxCmd} ${manager} install ${params} && ${npxCmd} lerna@${lernaVersion} bootstrap -- ${params}`;
    logger.debug({ cmd });
