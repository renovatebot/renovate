...
async function processBranch(branchConfig, prHourlyLimitReached, packageFiles) {
  ...
  try {
    ...
    if (config.stabilityThreshold) {
      ...
      const parsedLatestUpgradeTimestamp = DateTime.fromISO(
        config.latestUpgradeTimestamp
      );

      if (
        parsedStabilityThreshold.valueOf() +
          parsedLatestUpgradeTimestamp.diffNow().valueOf() >=
        0
      ) {
