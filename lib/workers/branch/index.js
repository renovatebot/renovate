...
async function writePackageFiles(config) {
...
async function ensureBranch(config) {
  ...
  await writePackageFiles(config);
  ...
  for (const upgrade of config.upgrades) {
    ...
    const tmpDir = path.join(
      config.tmpDir.name,
      path.dirname(upgrade.packageFile)
    );
    logger.debug(`upgrade tmpDir=${tmpDir}`);
    if (upgrade.type === 'lockFileMaintenance') {
      logger.debug('branch lockFileMaintenance');
      try {
        if (upgrade.hasYarnLock) {
          logger.debug('Maintaining yarn.lock');
          const newYarnLock = await yarn.generateLockFileNew(tmpDir, logger);
          if (newYarnLock) {
            commitFiles.push({
              name: path.join(path.dirname(upgrade.packageFile), 'yarn.lock'),
