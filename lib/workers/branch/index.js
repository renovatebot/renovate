...
async function processBranch(branchConfig, prHourlyLimitReached, packageFiles) {
  ...
  try {
    ...
    let isStableVersionReleaseTS = true;
    if (config.stabilityThreshold && config.releaseTimestamp) {
      const parsedStabilityThreshold = Duration.fromISO(
        config.stabilityThreshold
      );
      if (parsedStabilityThreshold.invalid) {
        throw new Error(
          `Stability threshold error: ${parsedStabilityThreshold.invalid.explanation}`
        );
      }

      const parsedReleaseTimestamp = DateTime.fromISO(config.releaseTimestamp);

      if (
        parsedStabilityThreshold.valueOf() +
          parsedReleaseTimestamp.diffNow().valueOf() >=
        0
      ) {
        isStableVersionReleaseTS = false;
      }
    }

    if (!isStableVersionReleaseTS) {
      logger.info(
        'Skipping branch creation due to failed stability threshold check'
      );
      return 'pending';
    }
