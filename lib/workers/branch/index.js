...
async function processBranch(branchConfig, prHourlyLimitReached, packageFiles) {
  ...
  const branchLogger = logger.child({
  ...
  branchLogger.debug(`branchExists=${branchExists}`);
  ...
  try {
    ...
    if (existingPr && !masterIssueCheck) {
      ...
      if (existingPr.state === 'closed') {
        ...
        if (branchExists) {
          ...
          if (config.dryRun) {
            branchLogger.info(
              'DRY-RUN: Would delete branch ' + config.branchName
            );
          } else {
    ...
    if (!branchExists && config.masterIssueApproval) {
      if (masterIssueCheck) {
        branchLogger.info(
          `Branch ${config.branchName} is approved for creation`
        );
      } else {
        branchLogger.info(`Branch ${config.branchName} needs approval`);
        return 'needs-approval';
      }
    }
    if (!branchExists && prHourlyLimitReached && !masterIssueCheck) {
      branchLogger.info('Reached PR creation limit - skipping branch creation');
      return 'pr-hourly-limit-reached';
    }
    if (branchExists) {
      branchLogger.debug('Checking if PR has been edited');
      if (branchPr) {
        branchLogger.debug('Found existing branch PR');
    ...
    if (!config.isScheduledNow && !masterIssueCheck) {
      if (!branchExists) {
        branchLogger.info('Skipping branch creation as not within schedule');
      ...
      if (config.updateNotScheduled === false && !prRebaseChecked) {
        branchLogger.debug('Skipping branch update as not within schedule');
        return 'not-scheduled';
      }
      // istanbul ignore if
      if (!branchPr) {
        branchLogger.debug('Skipping PR creation out of schedule');
        return 'not-scheduled';
      }
      branchLogger.debug(
    ...
    if (masterIssueCheck === 'rebase') {
      branchLogger.info('Manual rebase requested via master issue');
      delete config.parentBranch;
    } else {
      Object.assign(config, await getParentBranch(config));
    }
    branchLogger.debug(`Using parentBranch: ${config.parentBranch}`);
    ...
    if (config.updatedPackageFiles && config.updatedPackageFiles.length) {
      ...
      branchLogger.debug('No package files need updating');
    }
    ...
    if (config.updatedArtifacts && config.updatedArtifacts.length) {
      ...
      branchLogger.debug('No updated lock files in branch');
    }
    if (config.artifactErrors && config.artifactErrors.length) {
      if (config.releaseTimestamp) {
        branchLogger.debug(`Branch timestamp: ` + config.releaseTimestamp);
        ...
        if (releaseTimestamp.plus({ days: 1 }) < DateTime.local()) {
          ...
          branchLogger.info(
            'PR is less than a day old - raise error instead of PR'
          );
          throw new Error('lockfile-error');
        }
      } else {
        branchLogger.debug('PR has no releaseTimestamp');
      }
    }
    ...
    if (
      ...
      // istanbul ignore if
      if (config.dryRun) {
        branchLogger.info('DRY-RUN: Would delete lock file maintenance branch');
      } else {
        await platform.deleteBranch(config.branchName);
      }
      return 'done';
    ...
    if (branchExists || !config.requiresStatusChecks) {
      ...
      branchLogger.debug(`mergeStatus=${mergeStatus}`);
      if (mergeStatus === 'automerged') {
        branchLogger.debug('Branch is automerged - returning');
        return 'automerged';
      }
      if (
        ...
        branchLogger.info({ mergeStatus }, 'Branch automerge not possible');
        config.forcePr = true;
    ...
    if (err.message === 'rate-limit-exceeded') {
      branchLogger.debug('Passing rate-limit-exceeded error up');
      throw err;
    }
    if (err.message === 'repository-changed') {
      branchLogger.debug('Passing repository-changed error up');
      throw err;
    }
    if (
      err.message &&
      ...
      branchLogger.debug('Throwing bad credentials');
    ...
    if (err.message === 'bad-credentials') {
      branchLogger.debug('Passing bad-credentials error up');
      throw err;
    }
    if (err.message === 'integration-unauthorized') {
      branchLogger.debug('Passing integration-unauthorized error up');
      throw err;
    }
    if (err.message === 'lockfile-error') {
      branchLogger.debug('Passing lockfile-error up');
      throw err;
    }
    ...
    if (err.message.startsWith('disk-space')) {
      branchLogger.debug('Passing disk-space error up');
      throw err;
    }
    if (err.message.startsWith('Resource not accessible by integration')) {
      branchLogger.debug('Passing 403 error up');
      throw err;
    }
    if (err.message === 'update-failure') {
      branchLogger.warn('Error updating branch: update failure');
    } else if (
      ...
      branchLogger.error({ err }, `Error updating branch: ${err.message}`);
    }
    // Don't throw here - we don't want to stop the other renovations
    return 'error';
  }
  try {
    branchLogger.debug('Ensuring PR');
    branchLogger.debug(
    ...
    if (pr) {
      ...
      if (config.artifactErrors && config.artifactErrors.length) {
        ...
        if (existingState !== state) {
          branchLogger.debug(`Updating status check state to failed`);
          // istanbul ignore if
          if (config.dryRun) {
            branchLogger.info(
    ...
    if (
      ...
      branchLogger.debug('Passing PR error up');
      throw err;
    }
    // Otherwise don't throw here - we don't want to stop the other renovations
    branchLogger.error({ err }, `Error ensuring PR: ${err.message}`);
  }
  if (!branchExists) {
    return 'pr-created';
  }
