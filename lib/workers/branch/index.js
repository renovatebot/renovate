...
async function ensureBranch(config) {
  ...
  if (commitFiles.length) {
    ...
    logger.debug('Done committing files to branch');
  } else {
    logger.debug(`No files to commit`);
  }
  if ((await api.branchExists(branchName)) === false) {
    logger.debug('No branch exists');
  ...
  let context;
  let existingState;
  let removeStatusCheck;
  let state;
  let description;

  // lock file errors
  logger.debug('Checking lock-file branch status');
  context = 'renovate/lock-file';
  existingState = await api.getBranchStatusCheck(branchName, context);
  logger.debug(`Existing status is ${existingState}`);
  // If no error then any "failed" status check needs to be set to "success"
  removeStatusCheck = existingState === 'failure' && lockError === false;
  const addStatusCheck = lockError && existingState !== 'failure';
  if (addStatusCheck || removeStatusCheck) {
    // Set unpublishable status check
    state = lockError ? 'failure' : 'success';
    logger.debug(`Need to add status: ${state}`);
    description = lockError
      ? 'Lock file update failed'
      : 'Lock file update succeeded';
    await api.setBranchStatus(
      branchName,
      context,
      description,
      state,
      'https://github.com/singapore/renovate/blob/master/docs/status-checks.md#lock-file'
    );
  } else {
    logger.debug('Do not need to set branch status check');
  }

  // unpublish-safe
  context = 'renovate/unpublish-safe';
  existingState = await api.getBranchStatusCheck(branchName, context);
  // If status check was enabled and then is disabled, any "pending" status check needs to be set to "success"
  removeStatusCheck = existingState === 'pending' && !config.unpublishSafe;
  if (
    (config.unpublishSafe || removeStatusCheck) &&
    typeof unpublishable !== 'undefined'
  ) {
    // Set unpublishable status check
    state = unpublishable || removeStatusCheck ? 'success' : 'pending';
