...
async function processBranch(branchConfig, prHourlyLimitReached, packageFiles) {
  ...
  try {
      ...
      if (branchPr) {
        ...
        if (!branchPr.canRebase) {
          ...
          const prRebaseChecked =
            branchPr.body &&
            /* istanbul ignore next */ branchPr.body.includes(
              `- [x] <!-- ${appSlug}-rebase -->`
            );
    ...
    if (
      config.updateType === 'lockFileMaintenance' &&
      /* istanbul ignore next */ !config.committedFiles &&
      /* istanbul ignore next */ !config.parentBranch &&
      /* istanbul ignore next */ branchExists
    ) {
      logger.info(
