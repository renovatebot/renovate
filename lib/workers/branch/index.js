...
async function processBranchUpgrades(branchUpgrades, errors, warnings) {
  ...
  config.logger = config.logger.child({
  ...
  try {
    ...
    if (config.recreateClosed === false) {
      if (
        ...
        await api.checkForClosedPr(config.branchName, config.prTitle)
      ) {
        return;
      }
      ...
      if (await api.checkForClosedPr(config.branchName, legacyPrTitle)) {
        return;
      }
    }
    const branchCreated = await module.exports.ensureBranch(config);
    if (branchCreated) {
      const pr = await prWorker.ensurePr(config, errors, warnings);
      if (pr) {
