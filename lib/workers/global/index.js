...
async function start() {
  try {
    ...
    config.maxPrsPerRun = 1;
    let prsAlreadyCreated = 0;
    for (const repository of config.repositories) {
      console.log('for loop: prsAlreadyCreated: ' + prsAlreadyCreated);
      if (prsAlreadyCreated >= config.maxPrsPerRun) {
        console.log('max prs created this run');
        break;
      }
      const repoConfig = await getRepositoryConfig(config, repository);
      console.log(repoConfig.maxPrsPerRun);
      if (repoConfig.hostRules) {
        hostRules.clear();
        repoConfig.hostRules.forEach(rule => hostRules.add(rule));
        repoConfig.hostRules = [];
      }
      const repoResult = await repositoryWorker.renovateRepository(
        repoConfig,
        prsAlreadyCreated
      );
      prsAlreadyCreated += repoResult.prsCreated;
    }
    setMeta({});
    printStats();
