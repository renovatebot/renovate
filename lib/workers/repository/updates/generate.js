...
function generateBranchConfig(branchUpgrades) {
  ...
  branchUpgrades.forEach(upg => {
    ...
    if (!newVersions.includes(upg.newVersion)) {
      newVersions.push(upg.newVersion);
    }
  });
  const groupEligible =
  ...
  for (const branchUpgrade of branchUpgrades) {
    ...
    if (newVersions.length === 1) {
      [upgrade.singleVersion] = newVersions;
    } else {
    ...
    if (upgrade.baseBranchPrefix) {
      upgrade.branchPrefix += upgrade.baseBranchPrefix;
    }
    ...
    if (upgrade.baseBranches.length) {
      upgrade.prTitle += ` (${upgrade.baseBranch})`;
    }
