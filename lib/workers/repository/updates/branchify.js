...
const { flattenUpgrades } = require('../../../manager');
...
function branchifyUpgrades(input) {
  logger.debug('branchifyUpgrades');
  const config = flattenUpgrades(input);
  logger.debug(`${config.upgrades.length} upgrades found`);
  logger.trace({ upgrades: config.upgrades });
  const errors = [];
  const warnings = [];
