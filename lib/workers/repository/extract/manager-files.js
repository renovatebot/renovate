...
async function getMatchedFiles(manager, config, includePaths, ignorePaths) {
...
async function getPackageFiles(manager, config, matchedFiles) {
  const packageFiles = [];
...
async function getManagerPackageFiles(config, managerConfig) {
  ...
  const matchedFiles = await getMatchedFiles(
    manager,
    config,
    includePaths,
    ignorePaths
  ...
  return getPackageFiles(manager, config, matchedFiles);
}
