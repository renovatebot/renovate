  ...
  if (depConfig.ignoreDeps.includes(depName)) {
    ...
    if (depConfig.rangeStrategy === 'auto') {
      if (depType === 'devDependencies') {
        ...
        depConfig.rangeStrategy = 'widen';
      }
    }
    if (depConfig.rangeStrategy === 'auto') {
      // TODO: npm-specific logic should be in manager/npm
      if (manager === 'npm') {
        const semverParsed = parseRange(currentVersion);
        if (semverParsed.length > 1) {
          depConfig.rangeStrategy = 'widen';
        } else {
          depConfig.rangeStrategy = 'replace';
        }
      } else {
        depConfig.rangeStrategy = 'replace';
      }
