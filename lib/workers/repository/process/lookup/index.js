...
async function lookupUpdates(config) {
  ...
  if (isValid(currentValue)) {
    ...
    if (dependency.dockerRegistry) {
      ...
      res.dockerRepository = dependency.dockerRespository;
    }
  ...
  if (supportsDigests(config)) {
    ...
    for (const update of res.updates) {
      if (config.pinDigests || config.currentDigest) {
        const newValueIfneeded =
          config.manager === 'bazel' ? false : update.newValue;
        update.newDigest = await getDigest(config, newValueIfneeded);
        if (update.newDigest) {
