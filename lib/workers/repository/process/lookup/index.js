...
async function lookupUpdates(config) {
  ...
  if (isValid(currentValue)) {
    ...
    if (
      updateLockfileWithinRange &&
      lockedVersion &&
      rangeStrategy !== 'pin' /* && config.updateLockFiles */
    ) {
  ...
  for (const update of res.updates) {
    ...
    if (['bump', 'lockUpdate'].includes(updateType)) {
      update[updateType === 'bump' ? 'isBump' : 'isLockUpdate'] = true;
