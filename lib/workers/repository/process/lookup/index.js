...
async function lookupUpdates(config) {
  ...
  const { deprecationMessage, deprecationSource } = dependency;
  // istanbul ignore if
  if (deprecationMessage && config.raiseDeprecationWarnings) {
    logger.info(
      {
        deprecationSource,
        depName,
