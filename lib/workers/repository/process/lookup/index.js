...
async function lookupUpdates(config) {
  ...
  const {
    ...
    getNewValue,
    valueToVersion,
  } = versioning.get(config.versionScheme, config.versionConfig);
  /** @type any */
...
function getType(config, fromVersion, toVersion) {
  const { versionScheme, versionConfig, rangeStrategy, currentValue } = config;
  const { getMajor, getMinor, matches } = versioning.get(
    versionScheme,
    versionConfig
  );
...
function getFromVersion(config, rangeStrategy, allVersions) {
  const { currentValue, lockedVersion, versionScheme, versionConfig } = config;
  const {
    isSingleVersion,
    isVersion,
    maxSatisfyingVersion,
    minSatisfyingVersion,
  } = versioning.get(versionScheme, versionConfig);
