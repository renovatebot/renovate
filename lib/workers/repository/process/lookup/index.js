...
async function lookupUpdates(config) {
  ...
  if (isValid(currentValue)) {
    ...
    for (const toVersion of filteredVersions) {
      ...
      const release = dependency.releases.find(it => it.version === toVersion);
      if (release && Object.keys(release).length > 1) {
        for (const prop in release) {
          if (prop !== 'version' && prop !== 'gitRef') {
            update[prop] = release[prop];
          }
        }
      }
    }
