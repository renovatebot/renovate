...
describe('workers/repository/process/fetch', () => {
  describe('fetchUpdates()', () => {
    ...
    it('filters out replacement updates when target already exists as sibling dep', async () => {
      const packageFiles: Record<string, PackageFile[]> = {
        npm: [
          {
            packageFile: 'package.json',
      ...
      lookupUpdates.mockImplementation((cfg) => {
        if (cfg.depName === '@material-ui/core') {
          return {
            updates: [
              {
        ...
        return {
          updates: [{ updateType: 'minor', newValue: '^6.2.0' }],
        } as never;
      });
      await fetchUpdates(config, packageFiles);
      expect(packageFiles.npm[0].deps[0].updates).toHaveLength(0);
      expect(packageFiles.npm[0].deps[1].updates).toHaveLength(1);
    });

    it('keeps replacement updates when target does not exist as sibling', async () => {
      const packageFiles: Record<string, PackageFile[]> = {
        npm: [
          {
            packageFile: 'package.json',
      ...
      lookupUpdates.mockImplementation((cfg) => {
        ...
        return {
          updates: [{ updateType: 'minor', newValue: '^2.0.0' }],
        } as never;
      });
      await fetchUpdates(config, packageFiles);
      expect(packageFiles.npm[0].deps[0].updates).toHaveLength(1);
      expect(packageFiles.npm[0].deps[0].updates![0].updateType).toBe(
        'replacement',
      );
    });
  });
