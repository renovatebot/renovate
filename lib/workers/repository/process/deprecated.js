...
async function raiseDeprecationWarnings(config, packageFiles) {
  ...
  for (const [manager, files] of Object.entries(packageFiles)) {
    ...
    logger.debug(
      'Closing existing depricated package issues missing in current deprecatedPackages'
    );
    const issueList = await platform.getIssueList();
    const titleMatch = 'Dependency deprecation warning';
    if (issueList && issueList.length) {
      const depricatedIssues = issueList.filter(
        i => i.title.startsWith(titleMatch) && i.state === 'open'
      );
      for (const i of depricatedIssues) {
