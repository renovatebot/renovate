...
export default async function handleError(config, err) {
  ...
  if (err.message === errorTypes.NO_PACKAGE_FILES) {
  ...
  if (err.message === errorTypes.REGISTRY_FAILURE) {
  ...
  if (
    ...
    err.message === errorTypes.INSUFFICIENT_DISK_SPACE
  ) {
  ...
  if (err.message === errorTypes.AUTHENTICATION_ERROR) {
