...
async function getRenovatePrs(branchPrefix) {
  return (await platform.getPrList())
    ...
    .filter(
      pr => pr.title && pr.title.match(new RegExp(appStrings.appSlug, 'i'))
    );
}

async function getRenovateFiles(prNo) {
  return (await platform.getPrFiles(prNo)).filter(file =>
    appStrings.configFileNames.includes(file)
  );
}
...
async function validatePrs(config) {
  ...
  logger.debug(
    { renovatePrs },
    `Found ${renovatePrs.length} ${appStrings.appName} PRs`
  );
  let validations = [];
  for (const pr of renovatePrs) {
    try {
      ...
      let status;
      let description;
      const subject = `${appStrings.appName} Configuration Errors`;
      if (validations.length) {
        ...
        status = 'failure';
        description = `${appStrings.appName} config validation failed`; // GitHub limit
      } else {
        description = `${appStrings.appName} config is valid`;
        status = 'success';
      ...
      if (pr.sourceRepo === config.repository) {
        ...
        const context = `${appStrings.appSlug}/validate`;
