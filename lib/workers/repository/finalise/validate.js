...
async function validateRenovateFiles(validations, renovateFiles, pr, config) {
...
async function validateRenovatePrs(renovatePrs, config) {
  let validations = [];
  for (const pr of renovatePrs) {
    try {
      ...
      validations = await validateRenovateFiles(
        validations,
        renovateFiles,
        pr,
        config
...
async function validatePrs(config) {
  ...
  return validateRenovatePrs(renovatePrs, config);
}
