...
async function validatePrs(config) {
  const validateLogger = logger.child({ repository: config.repository });
  validateLogger.debug('branchPrefix: ' + config.branchPrefix);
  const renovatePrs = await getRenovatePrs(config.branchPrefix);
  validateLogger.debug(
    { renovatePrs },
    `Found ${renovatePrs.length} ${appName} PRs`
  );
  let validations = [];
  for (const pr of renovatePrs) {
    try {
        ...
        if (parsed) {
          const toValidate =
            file === 'package.json'
              ...
              : parsed;
          if (toValidate) {
            validateLogger.debug({ config: toValidate }, 'Validating config');
      ...
      if (pr.sourceRepo === config.repository) {
        ...
        validateLogger.debug('Skipping branch status for forked PR');
      }
    } catch (err) {
      // istanbul ignore if
      if (err.message === 'repository-changed') {
        validateLogger.info('Cannot access PR files to check them');
      } else {
        validateLogger.warn(
          {
            err,
