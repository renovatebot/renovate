...
async function renovateRepository(repoConfig, token) {
  ...
  try {
    ...
    if (config.packageFiles.length > 0) {
      config.provisionedPackageFiles = true;
    } else {
    ...
    config.logger.debug('Retrieving package.json renovate configs');
    ...
    for (let packageFile of config.packageFiles) {
      ...
      newPackageFiles.push(
        await apis.getPackageJsonConfig(config, packageFile)
      );
    }
    config.packageFiles = newPackageFiles;
