  ...
  try {
    ...
    if (config.baseBranches && config.baseBranches.length) {
      ...
      for (const [index, baseBranch] of commonConfig.baseBranches.entries()) {
        ...
        config = extractDependencies(config);
        config = await fetchUpdates(config);
      ...
      logger.debug('Single base branch');
      ...
      config = branchifyUpgrades(config, deps);
    }
    config = filterConfig(config, 'package');
