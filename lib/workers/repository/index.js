...
async function renovateRepository(repoConfig, token) {
  ...
  try {
    ...
    config.logger.debug('Resolving package files and content');
    config = await apis.resolvePackageFiles(config);
    config.logger.trace({ config }, 'post-packageFiles config');
    config.logger.debug('Resolving config presets');
    config = await presets.resolvePresets(config);
    config.logger.trace({ config }, 'post-presets config');
    ...
    if (!config.repoIsOnboarded) {
      ...
      config = await apis.resolvePackageFiles(config);
      config = presets.resolvePresets(config);
