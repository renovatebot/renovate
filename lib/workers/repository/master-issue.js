...
async function ensureMasterIssue(config, branches) {
  ...
  let issueBody = `This [master issue](https://renovatebot.com/blog/master-issue) contains a list of ${appStrings.appName} updates and their statuses.\n\n`;
  const pendingApprovals = branches.filter(
    branch => branch.res === 'needs-approval'
  );
  if (pendingApprovals.length) {
    issueBody += '## Pending Approval\n\n';
    issueBody += `These branches will be created by ${appStrings.appName} only once you click their checkbox below.\n\n`;
  ...
  if (prEdited.length) {
    ...
    issueBody += `These updates have been manually edited so ${appStrings.appName} will no longer make changes. To discard all commits and start over, check the box below.\n\n`;
