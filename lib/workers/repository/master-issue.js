...
function ensureMasterIssueSupported(config) {
  if ((config.isBitbucket || config.isAzure) && config.masterIssue) {
    const error = new Error('config-validation');
    error.validationError = 'Master issues not supported';
    error.validationMessage =
      'The selected platform does not currently support master issues.';
    throw error;
  }
}
