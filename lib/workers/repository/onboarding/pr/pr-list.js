...
function getPrList(config, branches) {
  ...
  prDesc += `With your current configuration, ${appName} will create ${
    branches.length
  } Pull Request`;
  ...
  for (const branch of branches) {
    ...
    prDesc += branch.baseBranch
      ? /* istanbul ignore next */ `  - Merge into: \`${branch.baseBranch}\`\n`
      : '';
    const seen = [];
    for (const upgrade of branch.upgrades) {
      ...
      if (upgrade.updateType === 'lockFileMaintenance') {
        ...
        text += upgrade.isLockfileUpdate
          ? /* istanbul ignore next */ ` to \`${upgrade.toVersion}\``
  ...
  if (
    ...
    } per hour, so it doesn't swamp any CI resources or spam the project. See docs for \`prhourlylimit\` for details.\n\n`;
  }
  return prDesc;
}
