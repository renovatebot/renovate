...
function generateConfig(branchUpgrades, logger) {
  const config = {
    upgrades: [],
  };
...
function groupByBranch(upgrades, logger) {
...
async function branchifyUpgrades(upgrades, parentLogger) {
  ...
  const branchConfigs = [];
  const res = module.exports.groupByBranch(upgrades, logger);
  for (const branchName of Object.keys(res.branchUpgrades)) {
    const branchUpgrades = res.branchUpgrades[branchName];
    const branchConfig = module.exports.generateConfig(branchUpgrades, logger);
    branchConfig.branchName = branchName;
    branchConfig.logger = logger.child({ branch: branchName });
