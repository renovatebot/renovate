...
// Manager
const dockerDetect = require('../../manager/docker/detect');
const npmDetect = require('../../manager/npm/detect');
...
async function detectPackageFiles(input) {
  ...
  if (config.meteor.enabled) {
    ...
    const allPackageJs = fileList.filter(
      file => file === 'package.js' || file.endsWith('/package.js')
    );
