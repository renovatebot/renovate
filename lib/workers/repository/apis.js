...
async function getPackageJsonConfig(config, packageFile) {
  config.logger.debug(
    { packageFile: packageFile.packageFile },
    'Retrieving package file'
  );
  config.logger.debug(`baseBranch=${config.baseBranch}`);
  const pFile = {};
  // save content
  pFile.content = await config.api.getFileJson(
    packageFile.packageFile,
    config.baseBranch
  );
  // hoist renovate config if exists
  if (pFile.content.renovate) {
    config.logger.debug(
      { packageFile: packageFile.packageFile },
      `Found renovate config`
    );
    pFile.hasRenovateConfig = true;
    Object.assign(pFile, migrateAndValidate(config, pFile.content.renovate));
    delete pFile.content.renovate;
  } else {
    config.logger.debug(
      { packageFile: packageFile.packageFile },
      `No renovate config`
  ...
  if (await config.api.getFileContent(yarnLockFileName, config.baseBranch)) {
    config.logger.debug(
      { packageFile: packageFile.packageFile },
      'Found yarn.lock'
    );
    pFile.hasYarnLock = true;
  } else {
    pFile.hasYarnLock = false;
  }
  const packageLockFileName = path.join(
    path.dirname(packageFile.packageFile),
    'package-lock.json'
  );
  if (await config.api.getFileContent(packageLockFileName, config.baseBranch)) {
    config.logger.debug(
      { packageFile: packageFile.packageFile },
      'Found yarn.lock'
    );
    pFile.hasPackageLock = true;
  } else {
    pFile.hasPackageLock = false;
  }
  return configParser.mergeChildConfig(packageFile, pFile);
}
