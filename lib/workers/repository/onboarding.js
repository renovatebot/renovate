...
async function checkForLerna(config) {
  ...
  const packageDirectories = await config.api.getSubDirectories('packages');
  if (packageDirectories.length === 0) {
    return {};
  }
  return { ignoreDeps: packageDirectories };
}
...
async function createBranch(config) {
  ...
  if (await module.exports.determineSemanticCommits(config)) {
    onboardingConfig.semanticCommits = true;
  }
  const lernaConfig = await module.exports.checkForLerna(config);
  Object.assign(onboardingConfig, lernaConfig);
  ...
  config.logger.debug({ config: onboardingConfig }, 'onboardingConfig');
