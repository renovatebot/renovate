...
async function isRepoPrivate(config) {
  ...
  for (const packageFile of config.packageFiles) {
    ...
    const packageContent = await api.getFileJson(fileName);
...
async function createBranch(config) {
  ...
  if (repoIsPrivate) {
    logger.debug('Repo is private - pinning dependencies versions');
  } else {
    logger.debug('Repo is not private - unpinning versions');
  ...
  const existingContent = await api.getFileContent(
    'renovate.json',
    onboardBranchName
  );
...
async function ensurePr(config, branchUpgrades) {
  const { api, logger, warnings, errors } = config;
...
`;
  ...
  const onboardBranchName = `${config.branchPrefix}configure`;
  const existingPr = await api.getBranchPr(onboardBranchName);
  if (existingPr) {
    ...
    await api.updatePr(existingPr.number, onboardPrTitle, prBody);
    logger.info(`Updated ${existingPr.displayNumber}`);
    return;
  }
  const pr = await api.createPr(onboardBranchName, onboardPrTitle, prBody);
...
async function getOnboardingStatus(config) {
  ...
  if (config.renovateJsonPresent) {
    logger.debug('Repo onboarded');
    return true;
  }
  const pr = await api.findPr(
  ...
  if (pr) {
    ...
    const prDetails = await api.getPr(pr.number);
