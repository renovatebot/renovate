...
async function ensurePr(upgrades) {
  ...
  if (config.automerge && config.automergeWhen === 'branch') {
    logger.debug('Checking if we can automerge branch');
    const branchStatus = await config.api.getBranchStatus(branchName);
    if (branchStatus === 'success') {
      logger.debug(`Automerging branch ${branchName}`);
      try {
        await config.api.mergeBranch(branchName);
      ...
      return null;
    } else if (branchStatus === 'pending' || branchStatus === 'running') {
      logger.debug(
        `Branch status is "${branchStatus}" - not ready for automerge`
      );
      return null;
    }
    logger.debug(
  ...
  } else if (config.prCreation === 'status-success') {
