  ...
  try {
    ...
    if (existingPr) {
      ...
      if (existingPr.title === prTitle && existingPr.body === prBody) {
        ...
        logger.debug(JSON.stringify(existingPr));
        if (config.automerge === 'any' ||
          (config.automerge === 'minor' && config.upgradeType === 'minor')) {
          logger.verbose('PR is candidate for automer');
          // Return if PR not ready for automerge
          if (existingPr.mergeable !== true) {
            logger.verbose('PR is not mergeable');
            return existingPr;
          }
          // Let's merge this
          logger.info(`Automerging #${existingPr.number}`);
          config.api.mergePr(existingPr.number, config.prMergeMethod);
        }
