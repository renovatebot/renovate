...
async function ensurePr(upgrades) {
  ...
  for (const upgrade of upgrades) {
    let logJSON = await changelogHelper.getChangeLogJSON(
      upgrade.depName,
      upgrade.changeLogFromVersion,
      upgrade.changeLogToVersion
    );
    logJSON.versions.forEach(version => {
      version.date = version.date.toISOString().slice(0,10);
      if (version.changes) {
        version.changes.forEach(change => {
          delete change.date;
          change.shortSha = change.sha.slice(0,7);
          change.commitUrl = `${logJSON.project.repository}/commit/${change.sha}`;
          if (change.message) {
            change.message = change.message.split('\n')[0];
          }
        });
      }
    });
  ...
  config.changelog = changelogHelper.getMarkdown(config.changelogs[0].logJSON);
