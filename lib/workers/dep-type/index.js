...
async function renovateNpmDepends(packageContent, config) {
  logger = config.logger || logger;
  logger.trace({ config }, `renovateNpmDepends(packageContent, config)`);
  // Extract all dependencies from the package.js
  const currentDeps = packageContent
