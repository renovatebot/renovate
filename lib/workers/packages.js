...
async function processPackageFile(repository, packageFile, packageConfig) {
  // Initialize globals
  config = Object.assign({}, packageConfig);
  config.packageFile = packageFile;
  logger = config.logger.child({ repository, packageFile }, true);

  logger.info(`Processing package file ${repository}:${packageFile}`);
