...
async function getRepositories(config) {
  ...
  try {
    ...
    for (const installation of installations) {
      ...
      logger.verbose(
        `Checking '${installation.account.login}' (id: ${installationId})`
      );
      const userToken = await ghApi.getInstallationToken(
        appToken,
        installationId
      );
      const userRepositories = await ghApi.getInstallationRepositories(
        userToken
      );
      logger.verbose(
        `Found ${userRepositories.repositories.length} repositories`
      );
      for (const repository of userRepositories.repositories) {
        installedRepos.push({
