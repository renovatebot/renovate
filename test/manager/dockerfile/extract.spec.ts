...
describe('lib/manager/dockerfile/extract', () => {
  describe('extractPackageFile()', () => {
    it('handles no FROM', () => {
      const res = extractPackageFile({ content: 'no from!' });
      expect(res).toBeNull();
    });
    it('handles naked dep', () => {
      const res = extractPackageFile({ content: 'FROM node\n' }).deps;
    ...
    it('handles tag', () => {
      const res = extractPackageFile({ content: 'FROM node:8.9.0-alpine\n' })
        .deps;
      expect(res).toMatchSnapshot();
    });
    it('handles digest', () => {
    ...
    it('handles from as', () => {
      const res = extractPackageFile({
        content: 'FROM node:8.9.0-alpine as base\n',
      }).deps;
      expect(res).toMatchSnapshot();
    });
    it('handles comments', () => {
    ...
    it('handles custom hosts', () => {
      const res = extractPackageFile({
        content: 'FROM registry2.something.info/node:8\n',
      }).deps;
      expect(res).toMatchSnapshot();
    });
    it('handles custom hosts and suffix', () => {
      const res = extractPackageFile({
        content: 'FROM registry2.something.info/node:8-alpine\n',
      }).deps;
      expect(res).toMatchSnapshot();
    });
    it('handles custom hosts with port', () => {
      const res = extractPackageFile({
        content: 'FROM registry2.something.info:5005/node:8\n',
      }).deps;
      expect(res).toMatchSnapshot();
    ...
    it('handles custom hosts with port without tag', () => {
      const res = extractPackageFile({
        content: 'FROM registry2.something.info:5005/node\n',
      }).deps;
      expect(res).toMatchSnapshot();
      expect(res[0].depName).toEqual('registry2.something.info:5005/node');
    });
    it('handles namespaced images', () => {
      const res = extractPackageFile({ content: 'FROM mynamespace/node:8\n' })
        .deps;
      expect(res).toMatchSnapshot();
    });
    it('handles custom hosts with namespace', () => {
      const res = extractPackageFile({
        content: 'FROM registry2.something.info/someaccount/node:8\n',
      }).deps;
      expect(res).toMatchSnapshot();
    });
    it('handles abnoral spacing', () => {
      const res = extractPackageFile({
        content: 'FROM    registry.allmine.info:5005/node:8.7.0\n\n',
      }).deps;
      expect(res).toMatchSnapshot();
    });
    ...
    it('skips scratchs', () => {
      const res = extractPackageFile({ content: 'FROM scratch\nADD foo\n' });
      expect(res).toBeNull();
    });
    ...
    it('extracts images on adjacent lines', () => {
      const res = extractPackageFile({ content: d1 }).deps;
      expect(res).toMatchSnapshot();
      expect(res).toHaveLength(2);
    });
    it('handles calico/node', () => {
      const res = extractPackageFile({ content: 'FROM calico/node\n' }).deps;
      expect(res).toMatchSnapshot();
    });
  });
});
